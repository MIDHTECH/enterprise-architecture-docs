# UC-GOV-014: Closed-Loop Automation
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | Closed-Loop Automation |
| Primary platform | Enterprise Cloud Governance and Operations Automation |
| Supporting use cases | [UC-GOV-012](UC-GOV-012-event-driven-remediation.md), [UC-GOV-013](UC-GOV-013-human-in-the-loop-remediation.md), [UC-GOV-011](UC-GOV-011-runbook-automation.md), [UC-OBS-014](../observability/UC-OBS-014-change-to-incident-correlation.md) |
...
| Jira epic | `EPIC-GOV-014` — Implement Closed-Loop Automation |
...
Closed-Loop Automation makes **Detect, remediate, validate and record recovery for low-risk failures** enforceable and reviewable without turning policy evaluation into unrestricted remediation.

For Closed-Loop Automation, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Detect, remediate, validate and record recovery for low-risk failures** on the documented
...
Acceptance for Closed-Loop Automation requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Cloud Governance and Operations Automation** owns the contract, control behavior, evidence schema, and recovery boundary for Closed-Loop Automation. |
...
- The implementation contract defines the scope required to demonstrate:
  **Detect, remediate, validate and record recovery for low-risk failures**
...
- the versioned contract for Closed-Loop Automation;
...
Rather than beginning with a product, explain Closed-Loop Automation by asking an engineer to
separate what is observed, what policy decides, who authorizes action and what evidence
survives. The result MidhHealth needs is to apply traceable controls to platform work that
supports provider and payer operations. Enterprise Cloud Governance and Operations Automation
...
escalate and who can override it. In this page, **UC-GOV-012: Event-Driven Remediation**
contributes explainable compliance or remediation decision with expiry and recovery state;
**UC-GOV-013: Human-in-the-Loop Remediation** contributes explainable compliance or remediation
...
Closed-Loop Automation is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Detect, remediate, validate and record recovery for low-risk failures**—rather than a new product or
environment.
...
Closed-Loop Automation. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-GOV-014 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-GOV-014/UC-GOV-014-architecture.svg)
...
Closed-Loop Automation remains accountable to its primary platform. The dependencies below
...
| Required upstream contract | [UC-GOV-013: Human-in-the-Loop Remediation](UC-GOV-013-human-in-the-loop-remediation.md) | explainable compliance or remediation decision with expiry and recovery state | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |
| Coordinated assurance handoff | [UC-GOV-011: Runbook Automation](UC-GOV-011-runbook-automation.md) | explainable compliance or remediation decision with expiry and recovery state | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |
...
Before Closed-Loop Automation is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For Closed-Loop Automation, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Detect, remediate, validate and record recovery for low-risk failures** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Closed-Loop Automation become
...
The Closed-Loop Automation design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Closed-Loop Automation | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Closed-Loop Automation, the primary threat is **a governance workflow receiving broader privileges than the control scope requires**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-GOV-014 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Closed-Loop Automation implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/security-governance/cloud-governance-ops-automation/contracts/uc-gov-014.yaml` |
| Primary implementation | `midhhealth/security-governance/cloud-governance-ops-automation/playbooks/closed-loop-automation.yml`; entry point: the `closed-loop-automation` control evaluator and bounded remediation entry point |
| Machine-readable result schema | `midhhealth/security-governance/cloud-governance-ops-automation/schemas/uc-gov-014-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/security-governance/cloud-governance-ops-automation/tests/fixtures/uc-gov-014/` |
| GitLab source gate | `midhhealth/security-governance/cloud-governance-ops-automation/.gitlab/ci/uc-gov-014.yml` |
| Operator diagnosis and recovery | `midhhealth/security-governance/cloud-governance-ops-automation/docs/runbooks/uc-gov-014.md` |
...
3. **Read-only proof:** execute the `closed-loop-automation` control evaluator and bounded remediation entry point, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/security-governance/cloud-governance-ops-automation/contracts/uc-gov-014.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/security-governance/cloud-governance-ops-automation/playbooks/closed-loop-automation.yml` | Primary implementation through the `closed-loop-automation` control evaluator and bounded remediation entry point | Planned |
| `midhhealth/security-governance/cloud-governance-ops-automation/schemas/uc-gov-014-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/security-governance/cloud-governance-ops-automation/tests/fixtures/uc-gov-014/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/security-governance/cloud-governance-ops-automation/.gitlab/ci/uc-gov-014.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/security-governance/cloud-governance-ops-automation/docs/runbooks/uc-gov-014.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-GOV-014-001: Define the Closed-Loop Automation contract
...
**Implementation steps:** Write `midhhealth/security-governance/cloud-governance-ops-automation/docs/runbooks/uc-gov-014.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-GOV-014-001A` contract and fixture review.

### STORY-GOV-014-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Closed-Loop Automation, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-GOV-014-002A` source validation and
`ART-GOV-014-002B` downstream-block proof.

### STORY-GOV-014-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Detect, remediate, validate and record recovery for low-risk failures** on its named scope without hidden
...
**Required attachments:** Future `ART-GOV-014-003A` bounded result,
`ART-GOV-014-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-GOV-014-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-GOV-014-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-GOV-014-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-GOV-014-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-GOV-014-003A` | Bounded observed result compared with **Detect, remediate, validate and record recovery for low-risk failures** | Approved existing execution path | Pending future execution |
| `ART-GOV-014-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Detect, remediate, validate and record recovery for low-risk failures | Detailed behavior and decision semantics documented |
