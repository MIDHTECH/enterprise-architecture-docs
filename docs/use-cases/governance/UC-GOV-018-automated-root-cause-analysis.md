# UC-GOV-018: Automated Root-Cause Analysis
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | Automated Root-Cause Analysis |
...
Automated Root-Cause Analysis makes **Telemetry, deployment and infrastructure changes are correlated for RCA** enforceable and reviewable without turning policy evaluation into unrestricted remediation.

For Automated Root-Cause Analysis, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Telemetry, deployment and infrastructure changes are correlated for RCA** on the documented
...
Acceptance for Automated Root-Cause Analysis requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Cloud Governance and Operations Automation** owns the contract, control behavior, evidence schema, and recovery boundary for Automated Root-Cause Analysis. |
...
- The implementation contract defines the scope required to demonstrate:
  **Telemetry, deployment and infrastructure changes are correlated for RCA**
...
- the versioned contract for Automated Root-Cause Analysis;
...
The architecture conversation for Automated Root-Cause Analysis should separate what is
...
escalate and who can override it. In this page, **UC-OBS-014: Change-to-Incident Correlation**
contributes traceable measurement or alert decision with owner and diagnostic context;
**UC-OBS-015: Automated Incident Triage** contributes traceable measurement or alert decision
with owner and diagnostic context. The first buildable boundary is existing GitLab runners, AWX
...
Automated Root-Cause Analysis is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Telemetry, deployment and infrastructure changes are correlated for RCA**—rather than a new product or
environment.
...
Automated Root-Cause Analysis. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-GOV-018 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-GOV-018/UC-GOV-018-architecture.svg)
...
Automated Root-Cause Analysis remains accountable to its primary platform. The dependencies below
provide explicit contracts or assurance evidence; they do not become alternate owners.

| Relationship | Use case | Required handoff | Failure propagation |
| --- | --- | --- | --- |
| Required upstream contract | [UC-OBS-014: Change-to-Incident Correlation](../observability/UC-OBS-014-change-to-incident-correlation.md) | traceable measurement or alert decision with owner and diagnostic context | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |
| Required upstream contract | [UC-OBS-015: Automated Incident Triage](../observability/UC-OBS-015-automated-incident-triage.md) | traceable measurement or alert decision with owner and diagnostic context | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |
...
Before Automated Root-Cause Analysis is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For Automated Root-Cause Analysis, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Telemetry, deployment and infrastructure changes are correlated for RCA** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Automated Root-Cause Analysis become
...
The Automated Root-Cause Analysis design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Automated Root-Cause Analysis | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Automated Root-Cause Analysis, the primary threat is **a governance workflow receiving broader privileges than the control scope requires**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-GOV-018 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Automated Root-Cause Analysis implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/security-governance/cloud-governance-ops-automation/contracts/uc-gov-018.yaml` |
| Primary implementation | `midhhealth/security-governance/cloud-governance-ops-automation/playbooks/automated-root-cause-analysis.yml`; entry point: the `automated-root-cause-analysis` control evaluator and bounded remediation entry point |
| Machine-readable result schema | `midhhealth/security-governance/cloud-governance-ops-automation/schemas/uc-gov-018-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/security-governance/cloud-governance-ops-automation/tests/fixtures/uc-gov-018/` |
| GitLab source gate | `midhhealth/security-governance/cloud-governance-ops-automation/.gitlab/ci/uc-gov-018.yml` |
| Operator diagnosis and recovery | `midhhealth/security-governance/cloud-governance-ops-automation/docs/runbooks/uc-gov-018.md` |
...
3. **Read-only proof:** execute the `automated-root-cause-analysis` control evaluator and bounded remediation entry point, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/security-governance/cloud-governance-ops-automation/contracts/uc-gov-018.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/security-governance/cloud-governance-ops-automation/playbooks/automated-root-cause-analysis.yml` | Primary implementation through the `automated-root-cause-analysis` control evaluator and bounded remediation entry point | Planned |
| `midhhealth/security-governance/cloud-governance-ops-automation/schemas/uc-gov-018-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/security-governance/cloud-governance-ops-automation/tests/fixtures/uc-gov-018/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/security-governance/cloud-governance-ops-automation/.gitlab/ci/uc-gov-018.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/security-governance/cloud-governance-ops-automation/docs/runbooks/uc-gov-018.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-GOV-018-001: Define the Automated Root-Cause Analysis contract
...
**Required attachments:** Future `ART-GOV-018-001A` contract and fixture review.

### STORY-GOV-018-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Automated Root-Cause Analysis, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-GOV-018-002A` source validation and
`ART-GOV-018-002B` downstream-block proof.

### STORY-GOV-018-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Telemetry, deployment and infrastructure changes are correlated for RCA** on its named scope without hidden
...
**Required attachments:** Future `ART-GOV-018-003A` bounded result,
`ART-GOV-018-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-GOV-018-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-GOV-018-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-GOV-018-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-GOV-018-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-GOV-018-003A` | Bounded observed result compared with **Telemetry, deployment and infrastructure changes are correlated for RCA** | Approved existing execution path | Pending future execution |
| `ART-GOV-018-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Telemetry, deployment and infrastructure changes are correlated for RCA | Detailed behavior and decision semantics documented |
