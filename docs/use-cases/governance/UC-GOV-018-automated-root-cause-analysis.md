...
**Automated Root-Cause Analysis** addresses a specific operating need inside the
Enterprise Cloud Governance and Operations Automation: **Telemetry, deployment and infrastructure changes are correlated for RCA**. Without a shared design, teams can perform
...
For an approved scope, the future workflow evaluates **Automated Root-Cause Analysis** through
...
| --- | --- |
| Owning platform | Automated Root-Cause Analysis turns a versioned platform intent into a repeatable decision rather than an isolated operator action. |
...
1. The request identifies **Automated Root-Cause Analysis**, the enterprise outcome, owner, immutable
...
```mermaid
flowchart LR
    Need["Enterprise need and owner"] --> Contract["Versioned Automated Root-Cause Analysis contract"]
...
These considerations make the page specific to **Automated Root-Cause Analysis** while preserving the
same enterprise control language used across the owning platform. In particular,
the later implementation must demonstrate signal quality, telemetry integrity, delivery provenance, control accountability.
...
| Planned `UC-GOV-018/contract` | Define inputs, owner, target allowlist, mode, coverage, policy, outputs, and safe stop |
| Planned `UC-GOV-018/result-schema` | Normalize provenance, observed values, decision, safety, recovery, and review fields |
| Planned `UC-GOV-018/fixtures` | Exercise passing, blocking, malformed, unauthorized, unavailable-dependency, and recovery cases |
...
**Description:** The platform owner and enterprise consumer need Automated Root-Cause Analysis defined
