...
Governance engineer, platform owner, security reviewer, and auditor need a repeatable, auditable implementation of **Automated Root-Cause Analysis**.
The canonical coverage target is: Telemetry, deployment and infrastructure changes are correlated for RCA. The work must strengthen the
...
evidence, and safe stop for Automated Root-Cause Analysis. CI proves source behavior with
...
| --- | --- |
| `use-cases/UC-GOV-018/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-GOV-018/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-GOV-018/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-GOV-018/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-GOV-018.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-GOV-018-001: Define and validate the platform contract

**Description:** The platform owner needs Automated Root-Cause Analysis expressed as versioned
...
sensitive inputs are rejected, and the expected coverage is: Telemetry, deployment and infrastructure changes are correlated for RCA.
...
**Required attachments:** `ART-GOV-018-001A` contract validation
and fixture report.

### STORY-GOV-018-002: Execute safely and prove the outcome

**Description:** Platform and enterprise owners need a reproducible result for
Automated Root-Cause Analysis that distinguishes source completion from runtime acceptance and
...
**Required attachments:** `ART-GOV-018-002A` execution result,
`ART-GOV-018-002B` rollback or non-mutation proof, and
`ATT-GOV-018-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-GOV-018-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-GOV-018-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-GOV-018-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-GOV-018-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Telemetry, deployment and infrastructure changes are correlated for RCA | Defined in canonical portfolio |
