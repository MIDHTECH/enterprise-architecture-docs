...
**Human-in-the-Loop Remediation** addresses a specific operating need inside the
Enterprise Cloud Governance and Operations Automation: **High-risk remediation pauses for approval before execution**. Without a shared design, teams can perform
...
For an approved scope, the future workflow evaluates **Human-in-the-Loop Remediation** through
...
| --- | --- |
| Owning platform | Human-in-the-Loop Remediation turns a versioned platform intent into a repeatable decision rather than an isolated operator action. |
...
1. The request identifies **Human-in-the-Loop Remediation**, the enterprise outcome, owner, immutable
...
```mermaid
flowchart LR
    Need["Enterprise need and owner"] --> Contract["Versioned Human-in-the-Loop Remediation contract"]
...
These considerations make the page specific to **Human-in-the-Loop Remediation** while preserving the
...
| Planned `UC-GOV-013/contract` | Define inputs, owner, target allowlist, mode, coverage, policy, outputs, and safe stop |
| Planned `UC-GOV-013/result-schema` | Normalize provenance, observed values, decision, safety, recovery, and review fields |
| Planned `UC-GOV-013/fixtures` | Exercise passing, blocking, malformed, unauthorized, unavailable-dependency, and recovery cases |
...
**Description:** The platform owner and enterprise consumer need Human-in-the-Loop Remediation defined
