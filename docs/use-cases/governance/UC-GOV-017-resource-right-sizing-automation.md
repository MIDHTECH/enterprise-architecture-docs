# UC-GOV-017: Resource Right-Sizing Automation
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | Resource Right-Sizing Automation |
| Primary platform | Enterprise Cloud Governance and Operations Automation |
| Supporting use cases | [UC-GOV-016](UC-GOV-016-cloud-cost-anomaly-detection.md), [UC-OBS-013](../observability/UC-OBS-013-cloud-native-monitoring.md), [UC-RSO-012](../resilience/UC-RSO-012-capacity-and-saturation-testing.md), [UC-INFRA-007](../infrastructure/UC-INFRA-007-infrastructure-change-impact-analysis.md) |
...
| Jira epic | `EPIC-GOV-017` — Implement Resource Right-Sizing Automation |
...
Resource Right-Sizing Automation makes **Utilization recommends CPU, memory, storage and replica adjustments** enforceable and reviewable without turning policy evaluation into unrestricted remediation.

For Resource Right-Sizing Automation, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Utilization recommends CPU, memory, storage and replica adjustments** on the documented
...
Acceptance for Resource Right-Sizing Automation requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Cloud Governance and Operations Automation** owns the contract, control behavior, evidence schema, and recovery boundary for Resource Right-Sizing Automation. |
...
- The implementation contract defines the scope required to demonstrate:
  **Utilization recommends CPU, memory, storage and replica adjustments**
...
- the versioned contract for Resource Right-Sizing Automation;
...
In practice, Resource Right-Sizing Automation makes sense when you separate what is observed,
...
loop is incomplete until the owner sees the effect. In this page, **UC-GOV-016: Cloud Cost
Anomaly Detection** contributes explainable compliance or remediation decision with expiry and
recovery state; **UC-OBS-013: Cloud-Native Monitoring** contributes traceable measurement or
alert decision with owner and diagnostic context. The first buildable boundary is existing
...
Resource Right-Sizing Automation is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Utilization recommends CPU, memory, storage and replica adjustments**—rather than a new product or
environment.
...
Resource Right-Sizing Automation. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-GOV-017 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-GOV-017/UC-GOV-017-architecture.svg)
...
Resource Right-Sizing Automation remains accountable to its primary platform. The dependencies below
provide explicit contracts or assurance evidence; they do not become alternate owners.

| Relationship | Use case | Required handoff | Failure propagation |
| --- | --- | --- | --- |
| Required upstream contract | [UC-GOV-016: Cloud Cost Anomaly Detection](UC-GOV-016-cloud-cost-anomaly-detection.md) | explainable compliance or remediation decision with expiry and recovery state | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |
| Required upstream contract | [UC-OBS-013: Cloud-Native Monitoring](../observability/UC-OBS-013-cloud-native-monitoring.md) | traceable measurement or alert decision with owner and diagnostic context | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |
| Coordinated assurance handoff | [UC-RSO-012: Capacity and Saturation Testing](../resilience/UC-RSO-012-capacity-and-saturation-testing.md) | readiness or exercise result tied to observed service recovery | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |
...
Before Resource Right-Sizing Automation is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For Resource Right-Sizing Automation, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Utilization recommends CPU, memory, storage and replica adjustments** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Resource Right-Sizing Automation become
...
The Resource Right-Sizing Automation design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Resource Right-Sizing Automation | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Resource Right-Sizing Automation, the primary threat is **a governance workflow receiving broader privileges than the control scope requires**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-GOV-017 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Resource Right-Sizing Automation implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/security-governance/cloud-governance-ops-automation/contracts/uc-gov-017.yaml` |
| Primary implementation | `midhhealth/security-governance/cloud-governance-ops-automation/playbooks/resource-right-sizing-automation.yml`; entry point: the `resource-right-sizing-automation` control evaluator and bounded remediation entry point |
| Machine-readable result schema | `midhhealth/security-governance/cloud-governance-ops-automation/schemas/uc-gov-017-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/security-governance/cloud-governance-ops-automation/tests/fixtures/uc-gov-017/` |
| GitLab source gate | `midhhealth/security-governance/cloud-governance-ops-automation/.gitlab/ci/uc-gov-017.yml` |
| Operator diagnosis and recovery | `midhhealth/security-governance/cloud-governance-ops-automation/docs/runbooks/uc-gov-017.md` |
...
3. **Read-only proof:** execute the `resource-right-sizing-automation` control evaluator and bounded remediation entry point, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/security-governance/cloud-governance-ops-automation/contracts/uc-gov-017.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/security-governance/cloud-governance-ops-automation/playbooks/resource-right-sizing-automation.yml` | Primary implementation through the `resource-right-sizing-automation` control evaluator and bounded remediation entry point | Planned |
| `midhhealth/security-governance/cloud-governance-ops-automation/schemas/uc-gov-017-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/security-governance/cloud-governance-ops-automation/tests/fixtures/uc-gov-017/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/security-governance/cloud-governance-ops-automation/.gitlab/ci/uc-gov-017.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/security-governance/cloud-governance-ops-automation/docs/runbooks/uc-gov-017.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-GOV-017-001: Define the Resource Right-Sizing Automation contract
...
**Implementation steps:** Write `midhhealth/security-governance/cloud-governance-ops-automation/docs/runbooks/uc-gov-017.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-GOV-017-001A` contract and fixture review.

### STORY-GOV-017-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Resource Right-Sizing Automation, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-GOV-017-002A` source validation and
`ART-GOV-017-002B` downstream-block proof.

### STORY-GOV-017-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Utilization recommends CPU, memory, storage and replica adjustments** on its named scope without hidden
...
**Required attachments:** Future `ART-GOV-017-003A` bounded result,
`ART-GOV-017-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-GOV-017-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-GOV-017-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-GOV-017-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-GOV-017-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-GOV-017-003A` | Bounded observed result compared with **Utilization recommends CPU, memory, storage and replica adjustments** | Approved existing execution path | Pending future execution |
| `ART-GOV-017-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Utilization recommends CPU, memory, storage and replica adjustments | Detailed behavior and decision semantics documented |
