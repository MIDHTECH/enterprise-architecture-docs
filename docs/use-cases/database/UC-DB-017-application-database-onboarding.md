# UC-DB-017: Application Database Onboarding
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | Application Database Onboarding |
...
| Jira epic | `EPIC-DB-017` — Implement Application Database Onboarding |
...
Application Database Onboarding defines the guarded database workflow for **Ownership, access, SLO and backup contract** on the existing PostgreSQL boundary.

For Application Database Onboarding, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Ownership, access, SLO and backup contract** on the documented
...
Acceptance for Application Database Onboarding requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Database Engineering and Reliability Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Application Database Onboarding. |
...
- The implementation contract defines the scope required to demonstrate:
  **Ownership, access, SLO and backup contract**
...
- the versioned contract for Application Database Onboarding;
...
In practice, Application Database Onboarding makes sense when you protect client compatibility
and data correctness while the database state changes underneath them. The result MidhHealth
needs is to keep enterprise transactional and operational data secure, performant, and
recoverable. Enterprise Database Engineering and Reliability Platform team owns the platform
...
Application Database Onboarding is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Ownership, access, SLO and backup contract**—rather than a new product or
environment.
...
Application Database Onboarding. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-DB-017 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-DB-017/UC-DB-017-architecture.svg)
...
Application Database Onboarding remains accountable to its primary platform. The dependencies below
...
Before Application Database Onboarding is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For Application Database Onboarding, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Ownership, access, SLO and backup contract** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Application Database Onboarding become
...
The Application Database Onboarding design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Application Database Onboarding | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Application Database Onboarding, the primary threat is **administrative credentials or row content escaping the database control boundary**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-DB-017 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Application Database Onboarding implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/data-and-integration/database-reliability-platform/contracts/uc-db-017.yaml` |
| Primary implementation | `midhhealth/data-and-integration/database-reliability-platform/playbooks/application-database-onboarding.yml`; entry point: the `application-database-onboarding` database playbook and report validator |
| Machine-readable result schema | `midhhealth/data-and-integration/database-reliability-platform/schemas/uc-db-017-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/data-and-integration/database-reliability-platform/tests/fixtures/uc-db-017/` |
| GitLab source gate | `midhhealth/data-and-integration/database-reliability-platform/.gitlab/ci/uc-db-017.yml` |
| Operator diagnosis and recovery | `midhhealth/data-and-integration/database-reliability-platform/docs/runbooks/uc-db-017.md` |
...
3. **Read-only proof:** execute the `application-database-onboarding` database playbook and report validator, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/data-and-integration/database-reliability-platform/contracts/uc-db-017.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/data-and-integration/database-reliability-platform/playbooks/application-database-onboarding.yml` | Primary implementation through the `application-database-onboarding` database playbook and report validator | Planned |
| `midhhealth/data-and-integration/database-reliability-platform/schemas/uc-db-017-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/data-and-integration/database-reliability-platform/tests/fixtures/uc-db-017/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/data-and-integration/database-reliability-platform/.gitlab/ci/uc-db-017.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/data-and-integration/database-reliability-platform/docs/runbooks/uc-db-017.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-DB-017-001: Define the Application Database Onboarding contract
...
**Required attachments:** Future `ART-DB-017-001A` contract and fixture review.

### STORY-DB-017-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Application Database Onboarding, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-DB-017-002A` source validation and
`ART-DB-017-002B` downstream-block proof.

### STORY-DB-017-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Ownership, access, SLO and backup contract** on its named scope without hidden
...
**Required attachments:** Future `ART-DB-017-003A` bounded result,
`ART-DB-017-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-DB-017-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-DB-017-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-DB-017-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-DB-017-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-DB-017-003A` | Bounded observed result compared with **Ownership, access, SLO and backup contract** | Approved existing execution path | Pending future execution |
| `ART-DB-017-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Ownership, access, SLO and backup contract | Detailed behavior and decision semantics documented |
