...
Database engineer, service owner, data owner, security reviewer, and SRE need a repeatable, auditable implementation of **Schema Migration Automation**.
The canonical coverage target is: Ordered, tested and reversible changes. The work must strengthen the
...
evidence, and safe stop for Schema Migration Automation. CI proves source behavior with
...
| --- | --- |
| `use-cases/UC-DB-004/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-DB-004/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-DB-004/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-DB-004/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-DB-004.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-DB-004-001: Define and validate the platform contract

**Description:** The platform owner needs Schema Migration Automation expressed as versioned
...
sensitive inputs are rejected, and the expected coverage is: Ordered, tested and reversible changes.
...
**Required attachments:** `ART-DB-004-001A` contract validation
and fixture report.

### STORY-DB-004-002: Execute safely and prove the outcome

**Description:** Platform and enterprise owners need a reproducible result for
Schema Migration Automation that distinguishes source completion from runtime acceptance and
...
**Required attachments:** `ART-DB-004-002A` execution result,
`ART-DB-004-002B` rollback or non-mutation proof, and
`ATT-DB-004-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-DB-004-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-DB-004-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-DB-004-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-DB-004-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Ordered, tested and reversible changes | Defined in canonical portfolio |
