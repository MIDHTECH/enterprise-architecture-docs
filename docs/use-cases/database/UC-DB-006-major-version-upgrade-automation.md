...
Database engineer, service owner, data owner, security reviewer, and SRE need a repeatable, auditable implementation of **Major-Version Upgrade Automation**.
The canonical coverage target is: Rehearsed upgrade using supported methods. The work must strengthen the
...
evidence, and safe stop for Major-Version Upgrade Automation. CI proves source behavior with
...
| --- | --- |
| `use-cases/UC-DB-006/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-DB-006/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-DB-006/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-DB-006/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-DB-006.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-DB-006-001: Define and validate the platform contract

**Description:** The platform owner needs Major-Version Upgrade Automation expressed as versioned
...
sensitive inputs are rejected, and the expected coverage is: Rehearsed upgrade using supported methods.
...
**Required attachments:** `ART-DB-006-001A` contract validation
and fixture report.

### STORY-DB-006-002: Execute safely and prove the outcome

**Description:** Platform and enterprise owners need a reproducible result for
Major-Version Upgrade Automation that distinguishes source completion from runtime acceptance and
...
**Required attachments:** `ART-DB-006-002A` execution result,
`ART-DB-006-002B` rollback or non-mutation proof, and
`ATT-DB-006-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-DB-006-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-DB-006-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-DB-006-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-DB-006-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Rehearsed upgrade using supported methods | Defined in canonical portfolio |
