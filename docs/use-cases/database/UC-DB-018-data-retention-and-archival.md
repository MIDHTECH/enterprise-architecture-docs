...
Database engineer, service owner, data owner, security reviewer, and SRE need a repeatable, auditable implementation of **Data Retention and Archival**.
The canonical coverage target is: Policy-driven lifecycle management. The work must strengthen the
...
| --- | --- |
| `use-cases/UC-DB-018/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-DB-018/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-DB-018/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-DB-018/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-DB-018.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-DB-018-001: Define and validate the platform contract
...
sensitive inputs are rejected, and the expected coverage is: Policy-driven lifecycle management.
...
**Required attachments:** `ART-DB-018-001A` contract validation
and fixture report.

### STORY-DB-018-002: Execute safely and prove the outcome
...
**Required attachments:** `ART-DB-018-002A` execution result,
`ART-DB-018-002B` rollback or non-mutation proof, and
`ATT-DB-018-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-DB-018-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-DB-018-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-DB-018-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-DB-018-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Policy-driven lifecycle management | Defined in canonical portfolio |
