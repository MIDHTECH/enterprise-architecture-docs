...
SRE, observability engineer, service owner, and incident responder need a repeatable, auditable implementation of **Deployment Health Scoring**.
The canonical coverage target is: Release health combines latency, errors, restarts, logs, traces and synthetic checks. The work must strengthen the
...
evidence, and safe stop for Deployment Health Scoring. CI proves source behavior with
...
| --- | --- |
| `use-cases/UC-OBS-008/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-OBS-008/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-OBS-008/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-OBS-008/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-OBS-008.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-OBS-008-001: Define and validate the platform contract

**Description:** The platform owner needs Deployment Health Scoring expressed as versioned
...
sensitive inputs are rejected, and the expected coverage is: Release health combines latency, errors, restarts, logs, traces and synthetic checks.
...
**Required attachments:** `ART-OBS-008-001A` contract validation
and fixture report.

### STORY-OBS-008-002: Execute safely and prove the outcome

**Description:** Platform and enterprise owners need a reproducible result for
Deployment Health Scoring that distinguishes source completion from runtime acceptance and
...
**Required attachments:** `ART-OBS-008-002A` execution result,
`ART-OBS-008-002B` rollback or non-mutation proof, and
`ATT-OBS-008-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-OBS-008-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-OBS-008-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-OBS-008-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-OBS-008-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Release health combines latency, errors, restarts, logs, traces and synthetic checks | Defined in canonical portfolio |
