...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | eBPF Observability |
| Primary platform | Enterprise Observability and SRE Reliability Platform |
| Supporting use cases | [UC-RSO-010](../resilience/UC-RSO-010-dependency-mapping.md), [UC-NET-025](../network/UC-NET-025-network-performance-monitoring.md), [UC-GOV-001](../governance/UC-GOV-001-compliance-evidence-collection.md), [UC-OBS-001](UC-OBS-001-slo-as-code.md) |
...
| Jira epic | `EPIC-OBS-005` — Implement eBPF Observability |
...
eBPF Observability turns **Kernel-level telemetry captures runtime behavior where code changes are not practical** into an owned operational signal rather than an unreviewed dashboard or alert.

For eBPF Observability, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Kernel-level telemetry captures runtime behavior where code changes are not practical** on the documented
...
Acceptance for eBPF Observability requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Observability and SRE Reliability Platform** owns the contract, control behavior, evidence schema, and recovery boundary for eBPF Observability. |
...
- The implementation contract defines the scope required to demonstrate:
  **Kernel-level telemetry captures runtime behavior where code changes are not practical**
...
- the versioned contract for eBPF Observability;
...
Rather than beginning with a product, explain eBPF Observability by asking an engineer to start
...
loop is incomplete until the owner sees the effect. In this page, **UC-RSO-010: Dependency
Mapping** contributes readiness or exercise result tied to observed service recovery;
**UC-NET-025: Network Performance Monitoring** contributes layered path decision with
before/after reachability and restore proof. The first buildable boundary is existing
Prometheus, Alertmanager, Grafana, Loki, Tempo, OpenTelemetry, Elastic, and GitLab/AWX paths.
The design stops at this rule: Reuse the existing lab; do not create a new monitoring VM,
telemetry backend, paging product, or unapproved data source.
...
eBPF Observability is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Kernel-level telemetry captures runtime behavior where code changes are not practical**—rather than a new product or
environment.
...
eBPF Observability. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-OBS-005 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-OBS-005/UC-OBS-005-architecture.svg)
...
eBPF Observability remains accountable to its primary platform. The dependencies below
...
| Required upstream contract | [UC-NET-025: Network Performance Monitoring](../network/UC-NET-025-network-performance-monitoring.md) | layered path decision with before/after reachability and restore proof | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |
...
Before eBPF Observability is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For eBPF Observability, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Kernel-level telemetry captures runtime behavior where code changes are not practical** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for eBPF Observability become
...
The eBPF Observability design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for eBPF Observability | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For eBPF Observability, the primary threat is **sensitive fields or credentials leaking into telemetry and diagnostic artifacts**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-OBS-005 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first eBPF Observability implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/reliability-operations/observability-sre-platform/contracts/uc-obs-005.yaml` |
| Primary implementation | `midhhealth/reliability-operations/observability-sre-platform/rules/ebpf-observability.yaml`; entry point: the `ebpf-observability` rule, query, or scoring evaluator |
| Machine-readable result schema | `midhhealth/reliability-operations/observability-sre-platform/schemas/uc-obs-005-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/reliability-operations/observability-sre-platform/tests/fixtures/uc-obs-005/` |
| GitLab source gate | `midhhealth/reliability-operations/observability-sre-platform/.gitlab/ci/uc-obs-005.yml` |
| Operator diagnosis and recovery | `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-005.md` |
...
3. **Read-only proof:** execute the `ebpf-observability` rule, query, or scoring evaluator, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/reliability-operations/observability-sre-platform/contracts/uc-obs-005.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/rules/ebpf-observability.yaml` | Primary implementation through the `ebpf-observability` rule, query, or scoring evaluator | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/schemas/uc-obs-005-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/tests/fixtures/uc-obs-005/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/.gitlab/ci/uc-obs-005.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-005.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-OBS-005-001: Define the eBPF Observability contract
...
**Implementation steps:** Write `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-005.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-OBS-005-001A` contract and fixture review.

### STORY-OBS-005-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates eBPF Observability, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-OBS-005-002A` source validation and
`ART-OBS-005-002B` downstream-block proof.

### STORY-OBS-005-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Kernel-level telemetry captures runtime behavior where code changes are not practical** on its named scope without hidden
...
**Required attachments:** Future `ART-OBS-005-003A` bounded result,
`ART-OBS-005-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-OBS-005-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-OBS-005-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-OBS-005-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-OBS-005-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-OBS-005-003A` | Bounded observed result compared with **Kernel-level telemetry captures runtime behavior where code changes are not practical** | Approved existing execution path | Pending future execution |
| `ART-OBS-005-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Kernel-level telemetry captures runtime behavior where code changes are not practical | Detailed behavior and decision semantics documented |
