# UC-OBS-004: Centralized Log Management
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | Centralized Log Management |
...
Centralized Log Management turns **Logs from pods, VMs, and services collected centrally** into an owned operational signal rather than an unreviewed dashboard or alert.

For Centralized Log Management, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Logs from pods, VMs, and services collected centrally** on the documented
...
Acceptance for Centralized Log Management requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Observability and SRE Reliability Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Centralized Log Management. |
...
- The implementation contract defines the scope required to demonstrate:
  **Logs from pods, VMs, and services collected centrally**
...
- the versioned contract for Centralized Log Management;
...
The architecture conversation for Centralized Log Management should start with the operator
...
identity or evidence from an earlier one. In this page, **UC-RSO-009: Service Ownership**
contributes readiness or exercise result tied to observed service recovery; **UC-DATA-015: Data
Classification** contributes validated data result with counts, lineage, quality, and
reconciliation state. The first buildable boundary is existing Prometheus, Alertmanager,
...
Centralized Log Management is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Logs from pods, VMs, and services collected centrally**—rather than a new product or
environment.
...
Centralized Log Management. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-OBS-004 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-OBS-004/UC-OBS-004-architecture.svg)
...
Centralized Log Management remains accountable to its primary platform. The dependencies below
...
| Required upstream contract | [UC-DATA-015: Data Classification](../data/UC-DATA-015-data-classification.md) | validated data result with counts, lineage, quality, and reconciliation state | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |
...
| Coordinated assurance handoff | [UC-RSO-006: Automated Incident Evidence Collection](../resilience/UC-RSO-006-automated-incident-evidence-collection.md) | readiness or exercise result tied to observed service recovery | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |

Before Centralized Log Management is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For Centralized Log Management, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Logs from pods, VMs, and services collected centrally** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Centralized Log Management become
...
The Centralized Log Management design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Centralized Log Management | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Centralized Log Management, the primary threat is **sensitive fields or credentials leaking into telemetry and diagnostic artifacts**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-OBS-004 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Centralized Log Management implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/reliability-operations/observability-sre-platform/contracts/uc-obs-004.yaml` |
| Primary implementation | `midhhealth/reliability-operations/observability-sre-platform/rules/centralized-log-management.yaml`; entry point: the `centralized-log-management` rule, query, or scoring evaluator |
| Machine-readable result schema | `midhhealth/reliability-operations/observability-sre-platform/schemas/uc-obs-004-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/reliability-operations/observability-sre-platform/tests/fixtures/uc-obs-004/` |
| GitLab source gate | `midhhealth/reliability-operations/observability-sre-platform/.gitlab/ci/uc-obs-004.yml` |
| Operator diagnosis and recovery | `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-004.md` |
...
3. **Read-only proof:** execute the `centralized-log-management` rule, query, or scoring evaluator, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/reliability-operations/observability-sre-platform/contracts/uc-obs-004.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/rules/centralized-log-management.yaml` | Primary implementation through the `centralized-log-management` rule, query, or scoring evaluator | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/schemas/uc-obs-004-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/tests/fixtures/uc-obs-004/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/.gitlab/ci/uc-obs-004.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-004.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-OBS-004-001: Define the Centralized Log Management contract
...
**Implementation steps:** Write `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-004.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-OBS-004-001A` contract and fixture review.

### STORY-OBS-004-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Centralized Log Management, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-OBS-004-002A` source validation and
`ART-OBS-004-002B` downstream-block proof.

### STORY-OBS-004-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Logs from pods, VMs, and services collected centrally** on its named scope without hidden
...
**Required attachments:** Future `ART-OBS-004-003A` bounded result,
`ART-OBS-004-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-OBS-004-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-OBS-004-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-OBS-004-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-OBS-004-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-OBS-004-003A` | Bounded observed result compared with **Logs from pods, VMs, and services collected centrally** | Approved existing execution path | Pending future execution |
| `ART-OBS-004-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Logs from pods, VMs, and services collected centrally | Detailed behavior and decision semantics documented |
