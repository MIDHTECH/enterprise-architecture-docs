...
| Supporting use cases | [UC-DB-008](../database/UC-DB-008-database-performance-monitoring.md), [UC-RSO-009](../resilience/UC-RSO-009-service-ownership.md), [UC-OBS-001](UC-OBS-001-slo-as-code.md), [UC-OBS-006](UC-OBS-006-alerting-and-on-call-notification.md) |
...
Rather than beginning with a product, explain Database Performance Monitoring by asking an
engineer to start with the operator decision the signal must support, then work backward to
trustworthy telemetry. The result MidhHealth needs is to turn existing telemetry into actionable
health and incident evidence for enterprise services. Enterprise Observability and SRE
Reliability Platform team owns the platform decision, while the consuming service or business
...
loop is incomplete until the owner sees the effect. In this page, **UC-DB-008: Database
Performance Monitoring** contributes bounded database result with before/after state and cleanup
or recovery proof; **UC-RSO-009: Service Ownership** contributes readiness or exercise result
tied to observed service recovery. The first buildable boundary is existing Prometheus,
...
| --- | --- | --- | --- |
| Required upstream contract | [UC-DB-008: Database Performance Monitoring](../database/UC-DB-008-database-performance-monitoring.md) | bounded database result with before/after state and cleanup or recovery proof | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |
