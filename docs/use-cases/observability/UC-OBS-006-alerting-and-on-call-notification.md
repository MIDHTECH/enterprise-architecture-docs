# UC-OBS-006: Alerting and On-Call Notification
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | Alerting and On-Call Notification |
...
| Jira epic | `EPIC-OBS-006` — Implement Alerting and On-Call Notification |
...
Alerting and On-Call Notification turns **Alerts route to incident channels or PagerDuty-style tools** into an owned operational signal rather than an unreviewed dashboard or alert.

For Alerting and On-Call Notification, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Alerts route to incident channels or PagerDuty-style tools** on the documented
...
Acceptance for Alerting and On-Call Notification requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Observability and SRE Reliability Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Alerting and On-Call Notification. |
...
- The implementation contract defines the scope required to demonstrate:
  **Alerts route to incident channels or PagerDuty-style tools**
...
- the versioned contract for Alerting and On-Call Notification;
...
For design review, walk through Alerting and On-Call Notification by trying to start with the
operator decision the signal must support, then work backward to trustworthy telemetry. The
result MidhHealth needs is to turn existing telemetry into actionable health and incident
...
Alerting and On-Call Notification is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Alerts route to incident channels or PagerDuty-style tools**—rather than a new product or
environment.
...
Alerting and On-Call Notification. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-OBS-006 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-OBS-006/UC-OBS-006-architecture.svg)
...
Alerting and On-Call Notification remains accountable to its primary platform. The dependencies below
...
Before Alerting and On-Call Notification is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For Alerting and On-Call Notification, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Alerts route to incident channels or PagerDuty-style tools** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Alerting and On-Call Notification become
...
The Alerting and On-Call Notification design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Alerting and On-Call Notification | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Alerting and On-Call Notification, the primary threat is **sensitive fields or credentials leaking into telemetry and diagnostic artifacts**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-OBS-006 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Alerting and On-Call Notification implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/reliability-operations/observability-sre-platform/contracts/uc-obs-006.yaml` |
| Primary implementation | `midhhealth/reliability-operations/observability-sre-platform/rules/alerting-and-on-call-notification.yaml`; entry point: the `alerting-and-on-call-notification` rule, query, or scoring evaluator |
| Machine-readable result schema | `midhhealth/reliability-operations/observability-sre-platform/schemas/uc-obs-006-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/reliability-operations/observability-sre-platform/tests/fixtures/uc-obs-006/` |
| GitLab source gate | `midhhealth/reliability-operations/observability-sre-platform/.gitlab/ci/uc-obs-006.yml` |
| Operator diagnosis and recovery | `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-006.md` |
...
3. **Read-only proof:** execute the `alerting-and-on-call-notification` rule, query, or scoring evaluator, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/reliability-operations/observability-sre-platform/contracts/uc-obs-006.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/rules/alerting-and-on-call-notification.yaml` | Primary implementation through the `alerting-and-on-call-notification` rule, query, or scoring evaluator | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/schemas/uc-obs-006-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/tests/fixtures/uc-obs-006/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/.gitlab/ci/uc-obs-006.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-006.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-OBS-006-001: Define the Alerting and On-Call Notification contract
...
**Implementation steps:** Write `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-006.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-OBS-006-001A` contract and fixture review.

### STORY-OBS-006-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Alerting and On-Call Notification, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-OBS-006-002A` source validation and
`ART-OBS-006-002B` downstream-block proof.

### STORY-OBS-006-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Alerts route to incident channels or PagerDuty-style tools** on its named scope without hidden
...
**Required attachments:** Future `ART-OBS-006-003A` bounded result,
`ART-OBS-006-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-OBS-006-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-OBS-006-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-OBS-006-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-OBS-006-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-OBS-006-003A` | Bounded observed result compared with **Alerts route to incident channels or PagerDuty-style tools** | Approved existing execution path | Pending future execution |
| `ART-OBS-006-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Alerts route to incident channels or PagerDuty-style tools | Detailed behavior and decision semantics documented |
