...
**OpenTelemetry Auto-Instrumentation** addresses a specific operating need inside the
Enterprise Observability and SRE Reliability Platform: **Standardized zero-touch metrics, logs and traces for supported workloads**. Without a shared design, teams can perform
...
For an approved scope, the future workflow evaluates **OpenTelemetry Auto-Instrumentation** through
...
| --- | --- |
| Owning platform | OpenTelemetry Auto-Instrumentation turns a versioned platform intent into a repeatable decision rather than an isolated operator action. |
...
1. The request identifies **OpenTelemetry Auto-Instrumentation**, the enterprise outcome, owner, immutable
...
```mermaid
flowchart LR
    Need["Enterprise need and owner"] --> Contract["Versioned OpenTelemetry Auto-Instrumentation contract"]
...
These considerations make the page specific to **OpenTelemetry Auto-Instrumentation** while preserving the
same enterprise control language used across the owning platform. In particular,
the later implementation must demonstrate telemetry integrity, operational signal.
...
| Planned `UC-OBS-003/contract` | Define inputs, owner, target allowlist, mode, coverage, policy, outputs, and safe stop |
| Planned `UC-OBS-003/result-schema` | Normalize provenance, observed values, decision, safety, recovery, and review fields |
| Planned `UC-OBS-003/fixtures` | Exercise passing, blocking, malformed, unauthorized, unavailable-dependency, and recovery cases |
...
**Description:** The platform owner and enterprise consumer need OpenTelemetry Auto-Instrumentation defined
