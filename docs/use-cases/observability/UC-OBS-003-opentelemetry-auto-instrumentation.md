# UC-OBS-003: OpenTelemetry Auto-Instrumentation
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | OpenTelemetry Auto-Instrumentation |
...
| Jira epic | `EPIC-OBS-003` — Implement OpenTelemetry Auto-Instrumentation |
...
OpenTelemetry Auto-Instrumentation turns **Standardized zero-touch metrics, logs and traces for supported workloads** into an owned operational signal rather than an unreviewed dashboard or alert.

For OpenTelemetry Auto-Instrumentation, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Standardized zero-touch metrics, logs and traces for supported workloads** on the documented
...
Acceptance for OpenTelemetry Auto-Instrumentation requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Observability and SRE Reliability Platform** owns the contract, control behavior, evidence schema, and recovery boundary for OpenTelemetry Auto-Instrumentation. |
...
- The implementation contract defines the scope required to demonstrate:
  **Standardized zero-touch metrics, logs and traces for supported workloads**
...
- the versioned contract for OpenTelemetry Auto-Instrumentation;
...
OpenTelemetry Auto-Instrumentation is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Standardized zero-touch metrics, logs and traces for supported workloads**—rather than a new product or
environment.
...
OpenTelemetry Auto-Instrumentation. It does not absorb the responsibilities of the dependency use cases
listed below.
...
flowchart LR
    ...
    subgraph Planned["Planned OpenTelemetry Auto-Instrumentation control"]
...
1. The request binds OpenTelemetry Auto-Instrumentation to immutable source, an inventory-resolved target,
...
- The canonical coverage test is: **Standardized zero-touch metrics, logs and traces for supported workloads**
...
OpenTelemetry Auto-Instrumentation remains accountable to its primary platform. The dependencies below
...
Before OpenTelemetry Auto-Instrumentation is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For OpenTelemetry Auto-Instrumentation, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Standardized zero-touch metrics, logs and traces for supported workloads** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for OpenTelemetry Auto-Instrumentation become
...
The OpenTelemetry Auto-Instrumentation design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for OpenTelemetry Auto-Instrumentation | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For OpenTelemetry Auto-Instrumentation, the primary threat is **sensitive fields or credentials leaking into telemetry and diagnostic artifacts**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-OBS-003 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first OpenTelemetry Auto-Instrumentation implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/reliability-operations/observability-sre-platform/contracts/uc-obs-003.yaml` |
| Primary implementation | `midhhealth/reliability-operations/observability-sre-platform/rules/opentelemetry-auto-instrumentation.yaml`; entry point: the `opentelemetry-auto-instrumentation` rule, query, or scoring evaluator |
| Machine-readable result schema | `midhhealth/reliability-operations/observability-sre-platform/schemas/uc-obs-003-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/reliability-operations/observability-sre-platform/tests/fixtures/uc-obs-003/` |
| GitLab source gate | `midhhealth/reliability-operations/observability-sre-platform/.gitlab/ci/uc-obs-003.yml` |
| Operator diagnosis and recovery | `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-003.md` |
...
3. **Read-only proof:** execute the `opentelemetry-auto-instrumentation` rule, query, or scoring evaluator, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/reliability-operations/observability-sre-platform/contracts/uc-obs-003.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/rules/opentelemetry-auto-instrumentation.yaml` | Primary implementation through the `opentelemetry-auto-instrumentation` rule, query, or scoring evaluator | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/schemas/uc-obs-003-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/tests/fixtures/uc-obs-003/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/.gitlab/ci/uc-obs-003.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-003.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-OBS-003-001: Define the OpenTelemetry Auto-Instrumentation contract
...
**Implementation steps:** Write `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-003.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-OBS-003-001A` contract and fixture review.

### STORY-OBS-003-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates OpenTelemetry Auto-Instrumentation, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-OBS-003-002A` source validation and
`ART-OBS-003-002B` downstream-block proof.

### STORY-OBS-003-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Standardized zero-touch metrics, logs and traces for supported workloads** on its named scope without hidden
...
**Required attachments:** Future `ART-OBS-003-003A` bounded result,
`ART-OBS-003-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-OBS-003-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-OBS-003-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-OBS-003-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-OBS-003-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-OBS-003-003A` | Bounded observed result compared with **Standardized zero-touch metrics, logs and traces for supported workloads** | Approved existing execution path | Pending future execution |
| `ART-OBS-003-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Standardized zero-touch metrics, logs and traces for supported workloads | Detailed behavior and decision semantics documented |
