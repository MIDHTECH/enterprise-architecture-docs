...
SRE, observability engineer, service owner, and incident responder need a repeatable, auditable implementation of **OpenTelemetry Auto-Instrumentation**.
The canonical coverage target is: Standardized zero-touch metrics, logs and traces for supported workloads. The work must strengthen the
...
evidence, and safe stop for OpenTelemetry Auto-Instrumentation. CI proves source behavior with
...
| Primary actors | SRE, observability engineer, service owner, and incident responder |
...
- Execution uses existing Prometheus, Alertmanager, Grafana, Loki, Tempo, OpenTelemetry, Elastic, and GitLab/AWX paths.
...
rollback or safe stop. The page does not authorize a new monitoring VM, telemetry backend, paging product, or unapproved data source. Any new
...
| --- | --- |
| `use-cases/UC-OBS-003/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-OBS-003/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-OBS-003/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-OBS-003/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-OBS-003.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-OBS-003-001: Define and validate the platform contract

**Description:** The platform owner needs OpenTelemetry Auto-Instrumentation expressed as versioned
...
sensitive inputs are rejected, and the expected coverage is: Standardized zero-touch metrics, logs and traces for supported workloads.
...
**Required attachments:** `ART-OBS-003-001A` contract validation
and fixture report.

### STORY-OBS-003-002: Execute safely and prove the outcome

**Description:** Platform and enterprise owners need a reproducible result for
OpenTelemetry Auto-Instrumentation that distinguishes source completion from runtime acceptance and
...
**Required attachments:** `ART-OBS-003-002A` execution result,
`ART-OBS-003-002B` rollback or non-mutation proof, and
`ATT-OBS-003-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-OBS-003-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-OBS-003-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-OBS-003-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-OBS-003-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Standardized zero-touch metrics, logs and traces for supported workloads | Defined in canonical portfolio |
| Enterprise fit | turn existing telemetry into actionable health and incident evidence for enterprise services | Traceability specified; owner review pending |
...
- Copy this specification into `midhhealth/reliability-operations/observability-sre-platform` as the epic and story contract.
