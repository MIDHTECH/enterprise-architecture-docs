# UC-OBS-007: Production Incident Troubleshooting Dashboard
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | Production Incident Troubleshooting Dashboard |
| Primary platform | Enterprise Observability and SRE Reliability Platform |
| Supporting use cases | [UC-RSO-004](../resilience/UC-RSO-004-incident-detection-and-classification.md), [UC-RSO-005](../resilience/UC-RSO-005-on-call-and-escalation-workflows.md), [UC-OBS-014](UC-OBS-014-change-to-incident-correlation.md), [UC-RSO-009](../resilience/UC-RSO-009-service-ownership.md) |
...
| Jira epic | `EPIC-OBS-007` — Implement Production Incident Troubleshooting Dashboard |
...
Production Incident Troubleshooting Dashboard turns **Single triage view for incidents** into an owned operational signal rather than an unreviewed dashboard or alert.

For Production Incident Troubleshooting Dashboard, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Single triage view for incidents** on the documented
...
Acceptance for Production Incident Troubleshooting Dashboard requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Observability and SRE Reliability Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Production Incident Troubleshooting Dashboard. |
...
- The implementation contract defines the scope required to demonstrate:
  **Single triage view for incidents**
...
- the versioned contract for Production Incident Troubleshooting Dashboard;
...
Production Incident Troubleshooting Dashboard is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Single triage view for incidents**—rather than a new product or
environment.
...
Production Incident Troubleshooting Dashboard. It does not absorb the responsibilities of the dependency use cases
listed below.
...
flowchart LR
    ...
    subgraph Planned["Planned Production Incident Troubleshooting Dashboard control"]
...
1. The request binds Production Incident Troubleshooting Dashboard to immutable source, an inventory-resolved target,
...
- The canonical coverage test is: **Single triage view for incidents**
...
Production Incident Troubleshooting Dashboard remains accountable to its primary platform. The dependencies below
...
| Coordinated assurance handoff | [UC-OBS-014: Change-to-Incident Correlation](UC-OBS-014-change-to-incident-correlation.md) | change identity linked to incident timing and symptoms | Missing, stale, or failed evidence blocks promotion or runtime action. |
...
Before Production Incident Troubleshooting Dashboard is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For Production Incident Troubleshooting Dashboard, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Single triage view for incidents** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Production Incident Troubleshooting Dashboard become
...
The Production Incident Troubleshooting Dashboard design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Production Incident Troubleshooting Dashboard | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Production Incident Troubleshooting Dashboard, the primary threat is **sensitive fields or credentials leaking into telemetry and diagnostic artifacts**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-OBS-007 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Production Incident Troubleshooting Dashboard implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/reliability-operations/observability-sre-platform/contracts/uc-obs-007.yaml` |
| Primary implementation | `midhhealth/reliability-operations/observability-sre-platform/rules/production-incident-troubleshooting-dashboard.yaml`; entry point: the `production-incident-troubleshooting-dashboard` rule, query, or scoring evaluator |
| Machine-readable result schema | `midhhealth/reliability-operations/observability-sre-platform/schemas/uc-obs-007-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/reliability-operations/observability-sre-platform/tests/fixtures/uc-obs-007/` |
| GitLab source gate | `midhhealth/reliability-operations/observability-sre-platform/.gitlab/ci/uc-obs-007.yml` |
| Operator diagnosis and recovery | `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-007.md` |
...
3. **Read-only proof:** execute the `production-incident-troubleshooting-dashboard` rule, query, or scoring evaluator, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/reliability-operations/observability-sre-platform/contracts/uc-obs-007.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/rules/production-incident-troubleshooting-dashboard.yaml` | Primary implementation through the `production-incident-troubleshooting-dashboard` rule, query, or scoring evaluator | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/schemas/uc-obs-007-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/tests/fixtures/uc-obs-007/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/.gitlab/ci/uc-obs-007.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-007.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-OBS-007-001: Define the Production Incident Troubleshooting Dashboard contract
...
**Implementation steps:** Write `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-007.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-OBS-007-001A` contract and fixture review.

### STORY-OBS-007-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Production Incident Troubleshooting Dashboard, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-OBS-007-002A` source validation and
`ART-OBS-007-002B` downstream-block proof.

### STORY-OBS-007-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Single triage view for incidents** on its named scope without hidden
...
**Required attachments:** Future `ART-OBS-007-003A` bounded result,
`ART-OBS-007-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-OBS-007-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-OBS-007-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-OBS-007-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-OBS-007-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-OBS-007-003A` | Bounded observed result compared with **Single triage view for incidents** | Approved existing execution path | Pending future execution |
| `ART-OBS-007-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Single triage view for incidents | Detailed behavior and decision semantics documented |
