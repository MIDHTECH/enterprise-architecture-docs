...
**Burn-Rate Alerting** addresses a specific operating need inside the
Enterprise Observability and SRE Reliability Platform: **Fast and slow error-budget burn alerts replace noisy symptom-only paging**. Without a shared design, teams can perform
...
For an approved scope, the future workflow evaluates **Burn-Rate Alerting** through
...
| --- | --- |
| Owning platform | Burn-Rate Alerting turns a versioned platform intent into a repeatable decision rather than an isolated operator action. |
...
1. The request identifies **Burn-Rate Alerting**, the enterprise outcome, owner, immutable
...
```mermaid
flowchart LR
    Need["Enterprise need and owner"] --> Contract["Versioned Burn-Rate Alerting contract"]
...
These considerations make the page specific to **Burn-Rate Alerting** while preserving the
...
| Planned `UC-OBS-016/contract` | Define inputs, owner, target allowlist, mode, coverage, policy, outputs, and safe stop |
| Planned `UC-OBS-016/result-schema` | Normalize provenance, observed values, decision, safety, recovery, and review fields |
| Planned `UC-OBS-016/fixtures` | Exercise passing, blocking, malformed, unauthorized, unavailable-dependency, and recovery cases |
...
**Description:** The platform owner and enterprise consumer need Burn-Rate Alerting defined
