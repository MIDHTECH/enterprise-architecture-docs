...
| Jira epic | `EPIC-OBS-016` — Implement Burn-Rate Alerting |
...
Burn-Rate Alerting turns **Fast and slow error-budget burn alerts replace noisy symptom-only paging** into an owned operational signal rather than an unreviewed dashboard or alert.

For Burn-Rate Alerting, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Fast and slow error-budget burn alerts replace noisy symptom-only paging** on the documented
...
Acceptance for Burn-Rate Alerting requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Observability and SRE Reliability Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Burn-Rate Alerting. |
...
- The implementation contract defines the scope required to demonstrate:
  **Fast and slow error-budget burn alerts replace noisy symptom-only paging**
...
- the versioned contract for Burn-Rate Alerting;
...
unit is the governed outcome—**Fast and slow error-budget burn alerts replace noisy symptom-only paging**—rather than a new product or
environment.
...
Burn-Rate Alerting. It does not absorb the responsibilities of the dependency use cases
listed below.
...
flowchart LR
    ...
    subgraph Planned["Planned Burn-Rate Alerting control"]
...
1. The request binds Burn-Rate Alerting to immutable source, an inventory-resolved target,
...
- The canonical coverage test is: **Fast and slow error-budget burn alerts replace noisy symptom-only paging**
...
Burn-Rate Alerting remains accountable to its primary platform. The dependencies below
...
For Burn-Rate Alerting, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Fast and slow error-budget burn alerts replace noisy symptom-only paging** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Burn-Rate Alerting become
...
The Burn-Rate Alerting design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Burn-Rate Alerting | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Burn-Rate Alerting, the primary threat is **sensitive fields or credentials leaking into telemetry and diagnostic artifacts**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-OBS-016 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Burn-Rate Alerting implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/reliability-operations/observability-sre-platform/contracts/uc-obs-016.yaml` |
| Primary implementation | `midhhealth/reliability-operations/observability-sre-platform/rules/burn-rate-alerting.yaml`; entry point: the `burn-rate-alerting` rule, query, or scoring evaluator |
| Machine-readable result schema | `midhhealth/reliability-operations/observability-sre-platform/schemas/uc-obs-016-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/reliability-operations/observability-sre-platform/tests/fixtures/uc-obs-016/` |
| GitLab source gate | `midhhealth/reliability-operations/observability-sre-platform/.gitlab/ci/uc-obs-016.yml` |
| Operator diagnosis and recovery | `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-016.md` |
...
3. **Read-only proof:** execute the `burn-rate-alerting` rule, query, or scoring evaluator, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/reliability-operations/observability-sre-platform/contracts/uc-obs-016.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/rules/burn-rate-alerting.yaml` | Primary implementation through the `burn-rate-alerting` rule, query, or scoring evaluator | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/schemas/uc-obs-016-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/tests/fixtures/uc-obs-016/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/.gitlab/ci/uc-obs-016.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-016.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-OBS-016-001: Define the Burn-Rate Alerting contract
...
**Implementation steps:** Write `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-016.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-OBS-016-001A` contract and fixture review.

### STORY-OBS-016-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Burn-Rate Alerting, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-OBS-016-002A` source validation and
`ART-OBS-016-002B` downstream-block proof.

### STORY-OBS-016-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Fast and slow error-budget burn alerts replace noisy symptom-only paging** on its named scope without hidden
...
**Required attachments:** Future `ART-OBS-016-003A` bounded result,
`ART-OBS-016-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-OBS-016-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-OBS-016-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-OBS-016-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-OBS-016-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-OBS-016-003A` | Bounded observed result compared with **Fast and slow error-budget burn alerts replace noisy symptom-only paging** | Approved existing execution path | Pending future execution |
| `ART-OBS-016-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Fast and slow error-budget burn alerts replace noisy symptom-only paging | Detailed behavior and decision semantics documented |
