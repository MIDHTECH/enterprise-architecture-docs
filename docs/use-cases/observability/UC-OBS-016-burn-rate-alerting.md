...
SRE, observability engineer, service owner, and incident responder need a repeatable, auditable implementation of **Burn-Rate Alerting**.
The canonical coverage target is: Fast and slow error-budget burn alerts replace noisy symptom-only paging. The work must strengthen the
...
evidence, and safe stop for Burn-Rate Alerting. CI proves source behavior with
...
| --- | --- |
| `use-cases/UC-OBS-016/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-OBS-016/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-OBS-016/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-OBS-016/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-OBS-016.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-OBS-016-001: Define and validate the platform contract

**Description:** The platform owner needs Burn-Rate Alerting expressed as versioned
...
sensitive inputs are rejected, and the expected coverage is: Fast and slow error-budget burn alerts replace noisy symptom-only paging.
...
**Required attachments:** `ART-OBS-016-001A` contract validation
and fixture report.

### STORY-OBS-016-002: Execute safely and prove the outcome

**Description:** Platform and enterprise owners need a reproducible result for
Burn-Rate Alerting that distinguishes source completion from runtime acceptance and
...
**Required attachments:** `ART-OBS-016-002A` execution result,
`ART-OBS-016-002B` rollback or non-mutation proof, and
`ATT-OBS-016-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-OBS-016-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-OBS-016-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-OBS-016-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-OBS-016-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Fast and slow error-budget burn alerts replace noisy symptom-only paging | Defined in canonical portfolio |
