...
flowchart LR
    subgraph Source["Existing source and intent boundary"]
        A["Service ownership, SLO, and telemetry definitions"]
    end
    subgraph Planned["Planned Kubernetes Cluster Health Monitoring control"]
        B["Rule and dashboard source validation"]
    ...
    subgraph Runtime["Existing approved execution boundary"]
        E["Existing observability collection path"]
        F["Inventoried services and telemetry sources"]
    end
    subgraph Assurance["Evidence and recovery boundary"]
        G["Signal quality, alert, dashboard, and SRE decision evidence"]
...
1. The request binds Kubernetes Cluster Health Monitoring to immutable source, an inventory-resolved target,
...
- The canonical coverage test is: **Dashboards for pods, nodes, namespaces, restarts, and capacity**
