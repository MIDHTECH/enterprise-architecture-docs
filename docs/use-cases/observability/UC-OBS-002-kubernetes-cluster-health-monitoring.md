...
SRE, observability engineer, service owner, and incident responder need a repeatable, auditable implementation of **Kubernetes Cluster Health Monitoring**.
The canonical coverage target is: Dashboards for pods, nodes, namespaces, restarts, and capacity. The work must strengthen the
...
evidence, and safe stop for Kubernetes Cluster Health Monitoring. CI proves source behavior with
...
| --- | --- |
| `use-cases/UC-OBS-002/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-OBS-002/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-OBS-002/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-OBS-002/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-OBS-002.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-OBS-002-001: Define and validate the platform contract

**Description:** The platform owner needs Kubernetes Cluster Health Monitoring expressed as versioned
...
sensitive inputs are rejected, and the expected coverage is: Dashboards for pods, nodes, namespaces, restarts, and capacity.
...
**Required attachments:** `ART-OBS-002-001A` contract validation
and fixture report.

### STORY-OBS-002-002: Execute safely and prove the outcome

**Description:** Platform and enterprise owners need a reproducible result for
Kubernetes Cluster Health Monitoring that distinguishes source completion from runtime acceptance and
...
**Required attachments:** `ART-OBS-002-002A` execution result,
`ART-OBS-002-002B` rollback or non-mutation proof, and
`ATT-OBS-002-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-OBS-002-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-OBS-002-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-OBS-002-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-OBS-002-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Dashboards for pods, nodes, namespaces, restarts, and capacity | Defined in canonical portfolio |
