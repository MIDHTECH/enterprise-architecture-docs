...
Synthetic Monitoring turns **External checks validate user-facing availability** into an owned operational signal rather than an unreviewed dashboard or alert.
...
The first delivery slice proves **External checks validate user-facing availability** on the documented
...
| --- | --- |
| Owning responsibility | **Enterprise Observability and SRE Reliability Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Synthetic Monitoring. |
...
- The implementation contract defines the scope required to demonstrate:
  **External checks validate user-facing availability**
...
A useful way for a new engineer to understand Synthetic Monitoring is to start with the operator
...
loop is incomplete until the owner sees the effect. In this page, **UC-RSO-011: Synthetic
Monitoring** contributes readiness or exercise result tied to observed service recovery;
...
Loki, Tempo, OpenTelemetry, Elastic, and GitLab/AWX paths. The design stops at this rule: Reuse
the existing lab; do not create a new monitoring VM, telemetry backend, paging product, or
...
unit is the governed outcome—**External checks validate user-facing availability**—rather than a new product or
environment.
...
![UC-OBS-012 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-OBS-012/UC-OBS-012-architecture.svg)
...
| --- | --- | --- | --- |
| Required upstream contract | [UC-RSO-011: Synthetic Monitoring](../resilience/UC-RSO-011-synthetic-monitoring.md) | readiness or exercise result tied to observed service recovery | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |
...
| --- | --- | --- |
| Functional correctness | Every required input is validated; **External checks validate user-facing availability** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
For Synthetic Monitoring, the primary threat is **sensitive fields or credentials leaking into telemetry and diagnostic artifacts**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-OBS-012 in
...
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/reliability-operations/observability-sre-platform/contracts/uc-obs-012.yaml` |
| Primary implementation | `midhhealth/reliability-operations/observability-sre-platform/rules/synthetic-monitoring.yaml`; entry point: the `synthetic-monitoring` rule, query, or scoring evaluator |
| Machine-readable result schema | `midhhealth/reliability-operations/observability-sre-platform/schemas/uc-obs-012-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/reliability-operations/observability-sre-platform/tests/fixtures/uc-obs-012/` |
| GitLab source gate | `midhhealth/reliability-operations/observability-sre-platform/.gitlab/ci/uc-obs-012.yml` |
| Operator diagnosis and recovery | `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-012.md` |
...
3. **Read-only proof:** execute the `synthetic-monitoring` rule, query, or scoring evaluator, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/reliability-operations/observability-sre-platform/contracts/uc-obs-012.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/rules/synthetic-monitoring.yaml` | Primary implementation through the `synthetic-monitoring` rule, query, or scoring evaluator | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/schemas/uc-obs-012-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/tests/fixtures/uc-obs-012/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/.gitlab/ci/uc-obs-012.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-012.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-OBS-012-001: Define the Synthetic Monitoring contract
...
**Implementation steps:** Write `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-012.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-OBS-012-001A` contract and fixture review.

### STORY-OBS-012-002: Build the source and evidence gate
...
**Required attachments:** Future `ART-OBS-012-002A` source validation and
`ART-OBS-012-002B` downstream-block proof.

### STORY-OBS-012-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **External checks validate user-facing availability** on its named scope without hidden
...
**Required attachments:** Future `ART-OBS-012-003A` bounded result,
`ART-OBS-012-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-OBS-012-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-OBS-012-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-OBS-012-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-OBS-012-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-OBS-012-003A` | Bounded observed result compared with **External checks validate user-facing availability** | Approved existing execution path | Pending future execution |
| `ART-OBS-012-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | External checks validate user-facing availability | Detailed behavior and decision semantics documented |
