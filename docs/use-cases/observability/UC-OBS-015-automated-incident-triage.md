...
**Automated Incident Triage** addresses a specific operating need inside the
Enterprise Observability and SRE Reliability Platform: **Triage output includes owner, dependency, dashboard, runbook and likely change source**. Without a shared design, teams can perform
...
For an approved scope, the future workflow evaluates **Automated Incident Triage** through
...
| --- | --- |
| Owning platform | Automated Incident Triage turns a versioned platform intent into a repeatable decision rather than an isolated operator action. |
...
1. The request identifies **Automated Incident Triage**, the enterprise outcome, owner, immutable
...
```mermaid
flowchart LR
    Need["Enterprise need and owner"] --> Contract["Versioned Automated Incident Triage contract"]
...
These considerations make the page specific to **Automated Incident Triage** while preserving the
same enterprise control language used across the owning platform. In particular,
the later implementation must demonstrate signal quality, operational signal.
...
| Planned `UC-OBS-015/contract` | Define inputs, owner, target allowlist, mode, coverage, policy, outputs, and safe stop |
| Planned `UC-OBS-015/result-schema` | Normalize provenance, observed values, decision, safety, recovery, and review fields |
| Planned `UC-OBS-015/fixtures` | Exercise passing, blocking, malformed, unauthorized, unavailable-dependency, and recovery cases |
...
**Description:** The platform owner and enterprise consumer need Automated Incident Triage defined
