# UC-OBS-015: Automated Incident Triage
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | Automated Incident Triage |
...
| Jira epic | `EPIC-OBS-015` — Implement Automated Incident Triage |
...
Automated Incident Triage turns **Triage output includes owner, dependency, dashboard, runbook and likely change source** into an owned operational signal rather than an unreviewed dashboard or alert.

For Automated Incident Triage, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Triage output includes owner, dependency, dashboard, runbook and likely change source** on the documented
...
Acceptance for Automated Incident Triage requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Observability and SRE Reliability Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Automated Incident Triage. |
...
- The implementation contract defines the scope required to demonstrate:
  **Triage output includes owner, dependency, dashboard, runbook and likely change source**
...
- the versioned contract for Automated Incident Triage;
...
unit is the governed outcome—**Triage output includes owner, dependency, dashboard, runbook and likely change source**—rather than a new product or
environment.
...
Automated Incident Triage. It does not absorb the responsibilities of the dependency use cases
listed below.
...
flowchart LR
    ...
    subgraph Planned["Planned Automated Incident Triage control"]
...
1. The request binds Automated Incident Triage to immutable source, an inventory-resolved target,
...
- The canonical coverage test is: **Triage output includes owner, dependency, dashboard, runbook and likely change source**
...
Automated Incident Triage remains accountable to its primary platform. The dependencies below
...
Before Automated Incident Triage is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For Automated Incident Triage, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Triage output includes owner, dependency, dashboard, runbook and likely change source** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Automated Incident Triage become
...
The Automated Incident Triage design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Automated Incident Triage | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Automated Incident Triage, the primary threat is **sensitive fields or credentials leaking into telemetry and diagnostic artifacts**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-OBS-015 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Automated Incident Triage implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/reliability-operations/observability-sre-platform/contracts/uc-obs-015.yaml` |
| Primary implementation | `midhhealth/reliability-operations/observability-sre-platform/rules/automated-incident-triage.yaml`; entry point: the `automated-incident-triage` rule, query, or scoring evaluator |
| Machine-readable result schema | `midhhealth/reliability-operations/observability-sre-platform/schemas/uc-obs-015-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/reliability-operations/observability-sre-platform/tests/fixtures/uc-obs-015/` |
| GitLab source gate | `midhhealth/reliability-operations/observability-sre-platform/.gitlab/ci/uc-obs-015.yml` |
| Operator diagnosis and recovery | `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-015.md` |
...
3. **Read-only proof:** execute the `automated-incident-triage` rule, query, or scoring evaluator, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/reliability-operations/observability-sre-platform/contracts/uc-obs-015.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/rules/automated-incident-triage.yaml` | Primary implementation through the `automated-incident-triage` rule, query, or scoring evaluator | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/schemas/uc-obs-015-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/tests/fixtures/uc-obs-015/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/.gitlab/ci/uc-obs-015.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-015.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-OBS-015-001: Define the Automated Incident Triage contract
...
**Implementation steps:** Write `midhhealth/reliability-operations/observability-sre-platform/docs/runbooks/uc-obs-015.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-OBS-015-001A` contract and fixture review.

### STORY-OBS-015-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Automated Incident Triage, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-OBS-015-002A` source validation and
`ART-OBS-015-002B` downstream-block proof.

### STORY-OBS-015-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Triage output includes owner, dependency, dashboard, runbook and likely change source** on its named scope without hidden
...
**Required attachments:** Future `ART-OBS-015-003A` bounded result,
`ART-OBS-015-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-OBS-015-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-OBS-015-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-OBS-015-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-OBS-015-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-OBS-015-003A` | Bounded observed result compared with **Triage output includes owner, dependency, dashboard, runbook and likely change source** | Approved existing execution path | Pending future execution |
| `ART-OBS-015-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Triage output includes owner, dependency, dashboard, runbook and likely change source | Detailed behavior and decision semantics documented |
