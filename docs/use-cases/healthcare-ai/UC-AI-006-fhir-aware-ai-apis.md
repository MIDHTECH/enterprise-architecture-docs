...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | FHIR-Aware AI APIs |
| Primary platform | Enterprise Healthcare AI Platform |
| Supporting use cases | [UC-DATA-007](../data/UC-DATA-007-schema-registry-and-evolution.md), [UC-DATA-008](../data/UC-DATA-008-event-contract-management.md), [UC-DATA-023](../data/UC-DATA-023-data-access-governance.md), [UC-DATA-015](../data/UC-DATA-015-data-classification.md) |
...
| Jira epic | `EPIC-AI-006` — Implement FHIR-Aware AI APIs |
...
FHIR-Aware AI APIs defines the healthcare-safe behavior needed for **Auditable clinical-data exchange for AI workflows** before AI output can influence an enterprise workflow.

For FHIR-Aware AI APIs, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Auditable clinical-data exchange for AI workflows** on the documented
...
Acceptance for FHIR-Aware AI APIs requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Healthcare AI Platform** owns the contract, control behavior, evidence schema, and recovery boundary for FHIR-Aware AI APIs. |
...
- The implementation contract defines the scope required to demonstrate:
  **Auditable clinical-data exchange for AI workflows**
...
- the versioned contract for FHIR-Aware AI APIs;
...
For design review, walk through FHIR-Aware AI APIs by trying to follow an approved question and
evidence source through retrieval, review and a bounded answer. The result MidhHealth needs is
to provide bounded and reviewable AI assistance without delegating regulated decisions.
...
**UC-DATA-008: Event-Contract Management** contributes event producer/consumer contract and
failure semantics. The first buildable boundary is existing GitLab shared runner, approved
...
FHIR-Aware AI APIs is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Auditable clinical-data exchange for AI workflows**—rather than a new product or
environment.
...
FHIR-Aware AI APIs. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-AI-006 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-AI-006/UC-AI-006-architecture.svg)
...
FHIR-Aware AI APIs remains accountable to its primary platform. The dependencies below
...
Before FHIR-Aware AI APIs is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For FHIR-Aware AI APIs, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Auditable clinical-data exchange for AI workflows** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for FHIR-Aware AI APIs become
...
The FHIR-Aware AI APIs design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for FHIR-Aware AI APIs | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For FHIR-Aware AI APIs, the primary threat is **prompt, retrieved content, model output, or tool request crossing a data or authorization boundary**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-AI-006 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first FHIR-Aware AI APIs implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/ai-and-ml-platform/healthcare-ai-platform/contracts/uc-ai-006.yaml` |
| Primary implementation | `midhhealth/ai-and-ml-platform/healthcare-ai-platform/src/evaluations/fhir-aware-ai-apis.py`; entry point: the `evaluate_fhir_aware_ai_apis` offline evaluator |
| Machine-readable result schema | `midhhealth/ai-and-ml-platform/healthcare-ai-platform/schemas/uc-ai-006-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/ai-and-ml-platform/healthcare-ai-platform/tests/fixtures/uc-ai-006/` |
| GitLab source gate | `midhhealth/ai-and-ml-platform/healthcare-ai-platform/.gitlab/ci/uc-ai-006.yml` |
| Operator diagnosis and recovery | `midhhealth/ai-and-ml-platform/healthcare-ai-platform/docs/runbooks/uc-ai-006.md` |
...
3. **Read-only proof:** execute the `evaluate_fhir_aware_ai_apis` offline evaluator, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/ai-and-ml-platform/healthcare-ai-platform/contracts/uc-ai-006.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/ai-and-ml-platform/healthcare-ai-platform/src/evaluations/fhir-aware-ai-apis.py` | Primary implementation through the `evaluate_fhir_aware_ai_apis` offline evaluator | Planned |
| `midhhealth/ai-and-ml-platform/healthcare-ai-platform/schemas/uc-ai-006-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/ai-and-ml-platform/healthcare-ai-platform/tests/fixtures/uc-ai-006/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/ai-and-ml-platform/healthcare-ai-platform/.gitlab/ci/uc-ai-006.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/ai-and-ml-platform/healthcare-ai-platform/docs/runbooks/uc-ai-006.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-AI-006-001: Define the FHIR-Aware AI APIs contract
...
**Implementation steps:** Write `midhhealth/ai-and-ml-platform/healthcare-ai-platform/docs/runbooks/uc-ai-006.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-AI-006-001A` contract and fixture review.

### STORY-AI-006-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates FHIR-Aware AI APIs, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-AI-006-002A` source validation and
`ART-AI-006-002B` downstream-block proof.

### STORY-AI-006-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Auditable clinical-data exchange for AI workflows** on its named scope without hidden
...
**Required attachments:** Future `ART-AI-006-003A` bounded result,
`ART-AI-006-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-AI-006-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-AI-006-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-AI-006-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-AI-006-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-AI-006-003A` | Bounded observed result compared with **Auditable clinical-data exchange for AI workflows** | Approved existing execution path | Pending future execution |
| `ART-AI-006-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Auditable clinical-data exchange for AI workflows | Detailed behavior and decision semantics documented |
