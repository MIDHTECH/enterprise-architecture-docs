# UC-AI-002: Clinical AI Assistant Platform
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | Clinical AI Assistant Platform |
...
| Jira epic | `EPIC-AI-002` — Implement Clinical AI Assistant Platform |
...
Clinical AI Assistant Platform defines the healthcare-safe behavior needed for **Secure assistants for clinical workflow support** before AI output can influence an enterprise workflow.

For Clinical AI Assistant Platform, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Secure assistants for clinical workflow support** on the documented
...
Acceptance for Clinical AI Assistant Platform requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Healthcare AI Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Clinical AI Assistant Platform. |
...
- The implementation contract defines the scope required to demonstrate:
  **Secure assistants for clinical workflow support**
...
- the versioned contract for Clinical AI Assistant Platform;
...
unit is the governed outcome—**Secure assistants for clinical workflow support**—rather than a new product or
environment.
...
Clinical AI Assistant Platform. It does not absorb the responsibilities of the dependency use cases
listed below.
...
flowchart LR
    ...
    subgraph Planned["Planned Clinical AI Assistant Platform control"]
...
1. The request binds Clinical AI Assistant Platform to immutable source, an inventory-resolved target,
...
- The canonical coverage test is: **Secure assistants for clinical workflow support**
...
Clinical AI Assistant Platform remains accountable to its primary platform. The dependencies below
...
Before Clinical AI Assistant Platform is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For Clinical AI Assistant Platform, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Secure assistants for clinical workflow support** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Clinical AI Assistant Platform become
...
The Clinical AI Assistant Platform design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Clinical AI Assistant Platform | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Clinical AI Assistant Platform, the primary threat is **prompt, retrieved content, model output, or tool request crossing a data or authorization boundary**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-AI-002 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Clinical AI Assistant Platform implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.
...
| Operator diagnosis and recovery | `midhhealth/ai-and-ml-platform/healthcare-ai-platform/docs/runbooks/uc-ai-002.md` |
...
3. **Read-only proof:** execute the `evaluate_clinical_ai_assistant_platform` offline evaluator, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/ai-and-ml-platform/healthcare-ai-platform/contracts/uc-ai-002.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/ai-and-ml-platform/healthcare-ai-platform/src/evaluations/clinical-ai-assistant-platform.py` | Primary implementation through the `evaluate_clinical_ai_assistant_platform` offline evaluator | Planned |
| `midhhealth/ai-and-ml-platform/healthcare-ai-platform/schemas/uc-ai-002-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/ai-and-ml-platform/healthcare-ai-platform/tests/fixtures/uc-ai-002/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/ai-and-ml-platform/healthcare-ai-platform/.gitlab/ci/uc-ai-002.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/ai-and-ml-platform/healthcare-ai-platform/docs/runbooks/uc-ai-002.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-AI-002-001: Define the Clinical AI Assistant Platform contract
...
**Implementation steps:** Write `midhhealth/ai-and-ml-platform/healthcare-ai-platform/docs/runbooks/uc-ai-002.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-AI-002-001A` contract and fixture review.

### STORY-AI-002-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Clinical AI Assistant Platform, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-AI-002-002A` source validation and
`ART-AI-002-002B` downstream-block proof.

### STORY-AI-002-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Secure assistants for clinical workflow support** on its named scope without hidden
...
**Required attachments:** Future `ART-AI-002-003A` bounded result,
`ART-AI-002-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-AI-002-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-AI-002-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-AI-002-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-AI-002-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-AI-002-003A` | Bounded observed result compared with **Secure assistants for clinical workflow support** | Approved existing execution path | Pending future execution |
| `ART-AI-002-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Secure assistants for clinical workflow support | Detailed behavior and decision semantics documented |
