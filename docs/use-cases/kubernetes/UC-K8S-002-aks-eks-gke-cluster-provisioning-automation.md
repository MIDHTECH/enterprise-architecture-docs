# UC-K8S-002: AKS/EKS/GKE Cluster Provisioning Automation
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | AKS/EKS/GKE Cluster Provisioning Automation |
...
| Jira epic | `EPIC-K8S-002` — Implement AKS/EKS/GKE Cluster Provisioning Automation |
...
AKS/EKS/GKE Cluster Provisioning Automation establishes **Future scope: Terraform patterns remain planned; managed-cloud deployment and acceptance are deferred** for the existing application cluster while preserving namespace, policy, and recovery boundaries.

For AKS/EKS/GKE Cluster Provisioning Automation, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Future scope: Terraform patterns remain planned; managed-cloud deployment and acceptance are deferred** on the documented
...
Acceptance for AKS/EKS/GKE Cluster Provisioning Automation requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Kubernetes Platform with GitOps** owns the contract, control behavior, evidence schema, and recovery boundary for AKS/EKS/GKE Cluster Provisioning Automation. |
...
- The implementation contract defines the scope required to demonstrate:
  **Future scope: Terraform patterns remain planned; managed-cloud deployment and acceptance are deferred**
...
- the versioned contract for AKS/EKS/GKE Cluster Provisioning Automation;
...
AKS/EKS/GKE Cluster Provisioning Automation is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Future scope: Terraform patterns remain planned; managed-cloud deployment and acceptance are deferred**—rather than a new product or
environment.
...
AKS/EKS/GKE Cluster Provisioning Automation. It does not absorb the responsibilities of the dependency use cases
listed below.
...
flowchart LR
    subgraph Source["Existing source and intent boundary"]
        A["Reviewed manifests, Helm values, and policy"]
    end
    subgraph Planned["Planned AKS/EKS/GKE Cluster Provisioning Automation control"]
        B["GitLab manifest and policy gate"]
    ...
    subgraph Runtime["Existing approved execution boundary"]
        E["Approved cluster delivery path"]
        F["Existing application Kubernetes cluster"]
    end
    subgraph Assurance["Evidence and recovery boundary"]
        G["Desired/observed state and workload-health evidence"]
...
1. The request binds AKS/EKS/GKE Cluster Provisioning Automation to immutable source, an inventory-resolved target,
...
- The canonical coverage test is: **Future scope: Terraform patterns remain planned; managed-cloud deployment and acceptance are deferred**
...
AKS/EKS/GKE Cluster Provisioning Automation remains accountable to its primary platform. The dependencies below
...
Before AKS/EKS/GKE Cluster Provisioning Automation is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For AKS/EKS/GKE Cluster Provisioning Automation, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Future scope: Terraform patterns remain planned; managed-cloud deployment and acceptance are deferred** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for AKS/EKS/GKE Cluster Provisioning Automation become
...
The AKS/EKS/GKE Cluster Provisioning Automation design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for AKS/EKS/GKE Cluster Provisioning Automation | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For AKS/EKS/GKE Cluster Provisioning Automation, the primary threat is **a manifest escaping its namespace, identity, image, or network boundary**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-K8S-002 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first AKS/EKS/GKE Cluster Provisioning Automation implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/platform-engineering/kubernetes-platform-gitops/contracts/uc-k8s-002.yaml` |
| Primary implementation | `midhhealth/platform-engineering/kubernetes-platform-gitops/use-cases/aks-eks-gke-cluster-provisioning-automation/policy.yaml`; entry point: the `aks-eks-gke-cluster-provisioning-automation` validation and reconciliation entry point |
| Machine-readable result schema | `midhhealth/platform-engineering/kubernetes-platform-gitops/schemas/uc-k8s-002-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/platform-engineering/kubernetes-platform-gitops/tests/fixtures/uc-k8s-002/` |
| GitLab source gate | `midhhealth/platform-engineering/kubernetes-platform-gitops/.gitlab/ci/uc-k8s-002.yml` |
| Operator diagnosis and recovery | `midhhealth/platform-engineering/kubernetes-platform-gitops/docs/runbooks/uc-k8s-002.md` |
...
3. **Read-only proof:** execute the `aks-eks-gke-cluster-provisioning-automation` validation and reconciliation entry point, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/platform-engineering/kubernetes-platform-gitops/contracts/uc-k8s-002.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/platform-engineering/kubernetes-platform-gitops/use-cases/aks-eks-gke-cluster-provisioning-automation/policy.yaml` | Primary implementation through the `aks-eks-gke-cluster-provisioning-automation` validation and reconciliation entry point | Planned |
| `midhhealth/platform-engineering/kubernetes-platform-gitops/schemas/uc-k8s-002-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/platform-engineering/kubernetes-platform-gitops/tests/fixtures/uc-k8s-002/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/platform-engineering/kubernetes-platform-gitops/.gitlab/ci/uc-k8s-002.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/platform-engineering/kubernetes-platform-gitops/docs/runbooks/uc-k8s-002.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-K8S-002-001: Define the AKS/EKS/GKE Cluster Provisioning Automation contract
...
**Implementation steps:** Write `midhhealth/platform-engineering/kubernetes-platform-gitops/docs/runbooks/uc-k8s-002.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-K8S-002-001A` contract and fixture review.

### STORY-K8S-002-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates AKS/EKS/GKE Cluster Provisioning Automation, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-K8S-002-002A` source validation and
`ART-K8S-002-002B` downstream-block proof.

### STORY-K8S-002-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Future scope: Terraform patterns remain planned; managed-cloud deployment and acceptance are deferred** on its named scope without hidden
...
**Required attachments:** Future `ART-K8S-002-003A` bounded result,
`ART-K8S-002-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-K8S-002-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-K8S-002-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-K8S-002-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-K8S-002-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-K8S-002-003A` | Bounded observed result compared with **Future scope: Terraform patterns remain planned; managed-cloud deployment and acceptance are deferred** | Approved existing execution path | Pending future execution |
| `ART-K8S-002-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Future scope: Terraform patterns remain planned; managed-cloud deployment and acceptance are deferred | Detailed behavior and decision semantics documented |
