...
Kubernetes engineer, workload owner, security reviewer, and SRE need a repeatable, auditable implementation of **AKS/EKS/GKE Cluster Provisioning Automation**.
The canonical coverage target is: Future scope: Terraform patterns remain planned; managed-cloud deployment and acceptance are deferred. The work must strengthen the
...
evidence, and safe stop for AKS/EKS/GKE Cluster Provisioning Automation. CI proves source behavior with
...
| Primary actors | Kubernetes engineer, workload owner, security reviewer, and SRE |
...
- Execution uses existing four-node application cluster, jenkins-agent01, GitLab, Jenkins, and accepted storage and ingress.
...
rollback or safe stop. The page does not authorize a new cluster, node, VM, IP address, load balancer, storage system, or unapproved add-on. Any new
...
| --- | --- |
| `use-cases/UC-K8S-002/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-K8S-002/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-K8S-002/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-K8S-002/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-K8S-002.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-K8S-002-001: Define and validate the platform contract

**Description:** The platform owner needs AKS/EKS/GKE Cluster Provisioning Automation expressed as versioned
...
sensitive inputs are rejected, and the expected coverage is: Future scope: Terraform patterns remain planned; managed-cloud deployment and acceptance are deferred.
...
**Required attachments:** `ART-K8S-002-001A` contract validation
and fixture report.

### STORY-K8S-002-002: Execute safely and prove the outcome

**Description:** Platform and enterprise owners need a reproducible result for
AKS/EKS/GKE Cluster Provisioning Automation that distinguishes source completion from runtime acceptance and
...
**Required attachments:** `ART-K8S-002-002A` execution result,
`ART-K8S-002-002B` rollback or non-mutation proof, and
`ATT-K8S-002-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-K8S-002-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-K8S-002-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-K8S-002-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-K8S-002-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Future scope: Terraform patterns remain planned; managed-cloud deployment and acceptance are deferred | Defined in canonical portfolio |
| Enterprise fit | provide a controlled runtime for provider, payer, data, and platform workloads | Traceability specified; owner review pending |
...
- Copy this specification into `midhhealth/platform-engineering/kubernetes-platform-gitops` as the epic and story contract.
