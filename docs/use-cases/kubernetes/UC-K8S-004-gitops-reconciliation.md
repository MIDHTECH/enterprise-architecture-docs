# UC-K8S-004: GitOps Reconciliation
...
GitOps Reconciliation establishes **Argo CD or Flux restores approved desired state and records sync health** for the existing application cluster while preserving namespace, policy, and recovery boundaries.

For GitOps Reconciliation, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Argo CD or Flux restores approved desired state and records sync health** on the documented
...
Acceptance for GitOps Reconciliation requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Kubernetes Platform with GitOps** owns the contract, control behavior, evidence schema, and recovery boundary for GitOps Reconciliation. |
...
- The implementation contract defines the scope required to demonstrate:
  **Argo CD or Flux restores approved desired state and records sync health**
...
- the versioned contract for GitOps Reconciliation;
...
unit is the governed outcome—**Argo CD or Flux restores approved desired state and records sync health**—rather than a new product or
environment.
...
GitOps Reconciliation. It does not absorb the responsibilities of the dependency use cases
listed below.
...
flowchart LR
    ...
    subgraph Planned["Planned GitOps Reconciliation control"]
...
1. The request binds GitOps Reconciliation to immutable source, an inventory-resolved target,
...
- The canonical coverage test is: **Argo CD or Flux restores approved desired state and records sync health**
...
GitOps Reconciliation remains accountable to its primary platform. The dependencies below
...
For GitOps Reconciliation, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Argo CD or Flux restores approved desired state and records sync health** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for GitOps Reconciliation become
...
The GitOps Reconciliation design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for GitOps Reconciliation | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For GitOps Reconciliation, the primary threat is **a manifest escaping its namespace, identity, image, or network boundary**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-K8S-004 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first GitOps Reconciliation implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/platform-engineering/kubernetes-platform-gitops/contracts/uc-k8s-004.yaml` |
| Primary implementation | `midhhealth/platform-engineering/kubernetes-platform-gitops/use-cases/gitops-reconciliation/policy.yaml`; entry point: the `gitops-reconciliation` validation and reconciliation entry point |
| Machine-readable result schema | `midhhealth/platform-engineering/kubernetes-platform-gitops/schemas/uc-k8s-004-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/platform-engineering/kubernetes-platform-gitops/tests/fixtures/uc-k8s-004/` |
| GitLab source gate | `midhhealth/platform-engineering/kubernetes-platform-gitops/.gitlab/ci/uc-k8s-004.yml` |
| Operator diagnosis and recovery | `midhhealth/platform-engineering/kubernetes-platform-gitops/docs/runbooks/uc-k8s-004.md` |
...
3. **Read-only proof:** execute the `gitops-reconciliation` validation and reconciliation entry point, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/platform-engineering/kubernetes-platform-gitops/contracts/uc-k8s-004.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/platform-engineering/kubernetes-platform-gitops/use-cases/gitops-reconciliation/policy.yaml` | Primary implementation through the `gitops-reconciliation` validation and reconciliation entry point | Planned |
| `midhhealth/platform-engineering/kubernetes-platform-gitops/schemas/uc-k8s-004-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/platform-engineering/kubernetes-platform-gitops/tests/fixtures/uc-k8s-004/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/platform-engineering/kubernetes-platform-gitops/.gitlab/ci/uc-k8s-004.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/platform-engineering/kubernetes-platform-gitops/docs/runbooks/uc-k8s-004.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-K8S-004-001: Define the GitOps Reconciliation contract
...
**Implementation steps:** Write `midhhealth/platform-engineering/kubernetes-platform-gitops/docs/runbooks/uc-k8s-004.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-K8S-004-001A` contract and fixture review.

### STORY-K8S-004-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates GitOps Reconciliation, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-K8S-004-002A` source validation and
`ART-K8S-004-002B` downstream-block proof.

### STORY-K8S-004-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Argo CD or Flux restores approved desired state and records sync health** on its named scope without hidden
...
**Required attachments:** Future `ART-K8S-004-003A` bounded result,
`ART-K8S-004-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-K8S-004-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-K8S-004-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-K8S-004-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-K8S-004-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-K8S-004-003A` | Bounded observed result compared with **Argo CD or Flux restores approved desired state and records sync health** | Approved existing execution path | Pending future execution |
| `ART-K8S-004-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Argo CD or Flux restores approved desired state and records sync health | Detailed behavior and decision semantics documented |
