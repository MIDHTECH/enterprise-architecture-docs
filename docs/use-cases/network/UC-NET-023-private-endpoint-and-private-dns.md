...
Network engineer, service owner, platform engineer, security reviewer, and SRE need a repeatable, auditable implementation of **Private Endpoint and Private DNS**.
The canonical coverage target is: Non-public managed-service access. The work must strengthen the
...
evidence, and safe stop for Private Endpoint and Private DNS. CI proves source behavior with
...
| --- | --- |
| `use-cases/UC-NET-023/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-NET-023/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-NET-023/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-NET-023/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-NET-023.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-NET-023-001: Define and validate the platform contract

**Description:** The platform owner needs Private Endpoint and Private DNS expressed as versioned
...
sensitive inputs are rejected, and the expected coverage is: Non-public managed-service access.
...
**Required attachments:** `ART-NET-023-001A` contract validation
and fixture report.

### STORY-NET-023-002: Execute safely and prove the outcome

**Description:** Platform and enterprise owners need a reproducible result for
Private Endpoint and Private DNS that distinguishes source completion from runtime acceptance and
...
**Required attachments:** `ART-NET-023-002A` execution result,
`ART-NET-023-002B` rollback or non-mutation proof, and
`ATT-NET-023-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-NET-023-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-NET-023-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-NET-023-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-NET-023-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Non-public managed-service access | Defined in canonical portfolio |
