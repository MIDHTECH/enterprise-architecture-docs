...
**Network Performance Monitoring** addresses a specific operating need inside the
Enterprise Network Engineering and Automation Platform: **Latency, loss, throughput and saturation**. Without a shared design, teams can perform
...
For an approved scope, the future workflow evaluates **Network Performance Monitoring** through
...
| --- | --- |
| Owning platform | Network Performance Monitoring turns a versioned platform intent into a repeatable decision rather than an isolated operator action. |
...
1. The request identifies **Network Performance Monitoring**, the enterprise outcome, owner, immutable
...
```mermaid
flowchart LR
    Need["Enterprise need and owner"] --> Contract["Versioned Network Performance Monitoring contract"]
...
These considerations make the page specific to **Network Performance Monitoring** while preserving the
same enterprise control language used across the owning platform. In particular,
the later implementation must demonstrate measurement window and trade-off, connectivity safety, network change safety.
...
| Planned `UC-NET-025/contract` | Define inputs, owner, target allowlist, mode, coverage, policy, outputs, and safe stop |
| Planned `UC-NET-025/result-schema` | Normalize provenance, observed values, decision, safety, recovery, and review fields |
| Planned `UC-NET-025/fixtures` | Exercise passing, blocking, malformed, unauthorized, unavailable-dependency, and recovery cases |
...
**Description:** The platform owner and enterprise consumer need Network Performance Monitoring defined
