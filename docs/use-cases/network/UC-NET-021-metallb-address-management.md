# UC-NET-021: MetalLB Address Management
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | MetalLB Address Management |
...
| Jira epic | `EPIC-NET-021` — Implement MetalLB Address Management |
...
MetalLB Address Management protects service connectivity while delivering **Controlled service address pools** across inventoried network paths.

For MetalLB Address Management, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Controlled service address pools** on the documented
...
Acceptance for MetalLB Address Management requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Network Engineering and Automation Platform** owns the contract, control behavior, evidence schema, and recovery boundary for MetalLB Address Management. |
...
- The implementation contract defines the scope required to demonstrate:
  **Controlled service address pools**
...
- the versioned contract for MetalLB Address Management;
...
Rather than beginning with a product, explain MetalLB Address Management by asking an engineer
to trace the actual packet or request path and make every ownership boundary observable. The
result MidhHealth needs is to maintain trusted connectivity and service paths across the
existing lab. Enterprise Network Engineering and Automation Platform team owns the platform
...
MetalLB Address Management is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Controlled service address pools**—rather than a new product or
environment.
...
MetalLB Address Management. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-NET-021 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-NET-021/UC-NET-021-architecture.svg)
...
MetalLB Address Management remains accountable to its primary platform. The dependencies below
...
Before MetalLB Address Management is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For MetalLB Address Management, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Controlled service address pools** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for MetalLB Address Management become
...
The MetalLB Address Management design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for MetalLB Address Management | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For MetalLB Address Management, the primary threat is **a network test or change crossing its approved source, destination, protocol, or capture boundary**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-NET-021 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first MetalLB Address Management implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/platform-engineering/network-engineering-platform/contracts/uc-net-021.yaml` |
| Primary implementation | `midhhealth/platform-engineering/network-engineering-platform/playbooks/metallb-address-management.yml`; entry point: the `metallb-address-management` validation role and bounded change entry point |
| Machine-readable result schema | `midhhealth/platform-engineering/network-engineering-platform/schemas/uc-net-021-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/platform-engineering/network-engineering-platform/tests/fixtures/uc-net-021/` |
| GitLab source gate | `midhhealth/platform-engineering/network-engineering-platform/.gitlab/ci/uc-net-021.yml` |
| Operator diagnosis and recovery | `midhhealth/platform-engineering/network-engineering-platform/docs/runbooks/uc-net-021.md` |
...
3. **Read-only proof:** execute the `metallb-address-management` validation role and bounded change entry point, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/platform-engineering/network-engineering-platform/contracts/uc-net-021.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/platform-engineering/network-engineering-platform/playbooks/metallb-address-management.yml` | Primary implementation through the `metallb-address-management` validation role and bounded change entry point | Planned |
| `midhhealth/platform-engineering/network-engineering-platform/schemas/uc-net-021-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/platform-engineering/network-engineering-platform/tests/fixtures/uc-net-021/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/platform-engineering/network-engineering-platform/.gitlab/ci/uc-net-021.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/platform-engineering/network-engineering-platform/docs/runbooks/uc-net-021.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-NET-021-001: Define the MetalLB Address Management contract
...
**Implementation steps:** Write `midhhealth/platform-engineering/network-engineering-platform/docs/runbooks/uc-net-021.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-NET-021-001A` contract and fixture review.

### STORY-NET-021-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates MetalLB Address Management, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-NET-021-002A` source validation and
`ART-NET-021-002B` downstream-block proof.

### STORY-NET-021-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Controlled service address pools** on its named scope without hidden
...
**Required attachments:** Future `ART-NET-021-003A` bounded result,
`ART-NET-021-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-NET-021-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-NET-021-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-NET-021-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-NET-021-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-NET-021-003A` | Bounded observed result compared with **Controlled service address pools** | Approved existing execution path | Pending future execution |
| `ART-NET-021-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Controlled service address pools | Detailed behavior and decision semantics documented |
