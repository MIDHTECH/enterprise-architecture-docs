# UC-NET-020: Ingress and Egress Controls
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | Ingress and Egress Controls |
| Primary platform | Enterprise Network Engineering and Automation Platform |
| Supporting use cases | [UC-K8S-001](../kubernetes/UC-K8S-001-kubernetes-configuration-drift.md), [UC-NET-018](UC-NET-018-kubernetes-networking.md), [UC-K8S-006](../kubernetes/UC-K8S-006-kubernetes-security-baseline-implementation.md), [UC-NET-012](UC-NET-012-firewall-policy-management.md) |
...
| Jira epic | `EPIC-NET-020` — Implement Ingress and Egress Controls |
...
Ingress and Egress Controls protects service connectivity while delivering **Governed workload traffic paths** across inventoried network paths.

For Ingress and Egress Controls, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Governed workload traffic paths** on the documented
...
Acceptance for Ingress and Egress Controls requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Network Engineering and Automation Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Ingress and Egress Controls. |
...
- The implementation contract defines the scope required to demonstrate:
  **Governed workload traffic paths**
...
- the versioned contract for Ingress and Egress Controls;
...
The architecture conversation for Ingress and Egress Controls should trace the actual packet or
...
**UC-NET-018: Kubernetes Networking** contributes cluster network identity and service-path
contract. The first buildable boundary is existing DNS, NGINX, KVM bridges, Kubernetes
...
Ingress and Egress Controls is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Governed workload traffic paths**—rather than a new product or
environment.
...
Ingress and Egress Controls. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-NET-020 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-NET-020/UC-NET-020-architecture.svg)
...
Ingress and Egress Controls remains accountable to its primary platform. The dependencies below
...
| Required upstream contract | [UC-NET-018: Kubernetes Networking](UC-NET-018-kubernetes-networking.md) | cluster network identity and service-path contract | Missing, stale, or failed evidence blocks promotion or runtime action. |
...
| Coordinated assurance handoff | [UC-NET-012: Firewall Policy Management](UC-NET-012-firewall-policy-management.md) | ordered firewall intent and allowed/denied path matrix | Missing, stale, or failed evidence blocks promotion or runtime action. |

Before Ingress and Egress Controls is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For Ingress and Egress Controls, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Governed workload traffic paths** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Ingress and Egress Controls become
...
The Ingress and Egress Controls design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Ingress and Egress Controls | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Ingress and Egress Controls, the primary threat is **a network test or change crossing its approved source, destination, protocol, or capture boundary**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-NET-020 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Ingress and Egress Controls implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/platform-engineering/network-engineering-platform/contracts/uc-net-020.yaml` |
| Primary implementation | `midhhealth/platform-engineering/network-engineering-platform/playbooks/ingress-and-egress-controls.yml`; entry point: the `ingress-and-egress-controls` validation role and bounded change entry point |
| Machine-readable result schema | `midhhealth/platform-engineering/network-engineering-platform/schemas/uc-net-020-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/platform-engineering/network-engineering-platform/tests/fixtures/uc-net-020/` |
| GitLab source gate | `midhhealth/platform-engineering/network-engineering-platform/.gitlab/ci/uc-net-020.yml` |
| Operator diagnosis and recovery | `midhhealth/platform-engineering/network-engineering-platform/docs/runbooks/uc-net-020.md` |
...
3. **Read-only proof:** execute the `ingress-and-egress-controls` validation role and bounded change entry point, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/platform-engineering/network-engineering-platform/contracts/uc-net-020.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/platform-engineering/network-engineering-platform/playbooks/ingress-and-egress-controls.yml` | Primary implementation through the `ingress-and-egress-controls` validation role and bounded change entry point | Planned |
| `midhhealth/platform-engineering/network-engineering-platform/schemas/uc-net-020-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/platform-engineering/network-engineering-platform/tests/fixtures/uc-net-020/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/platform-engineering/network-engineering-platform/.gitlab/ci/uc-net-020.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/platform-engineering/network-engineering-platform/docs/runbooks/uc-net-020.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-NET-020-001: Define the Ingress and Egress Controls contract
...
**Required attachments:** Future `ART-NET-020-001A` contract and fixture review.

### STORY-NET-020-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Ingress and Egress Controls, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-NET-020-002A` source validation and
`ART-NET-020-002B` downstream-block proof.

### STORY-NET-020-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Governed workload traffic paths** on its named scope without hidden
...
**Required attachments:** Future `ART-NET-020-003A` bounded result,
`ART-NET-020-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-NET-020-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-NET-020-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-NET-020-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-NET-020-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-NET-020-003A` | Bounded observed result compared with **Governed workload traffic paths** | Approved existing execution path | Pending future execution |
| `ART-NET-020-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Governed workload traffic paths | Detailed behavior and decision semantics documented |
