...
**Authoritative and Recursive DNS** addresses a specific operating need inside the
Enterprise Network Engineering and Automation Platform: **Managed internal name resolution**. Without a shared design, teams can perform
...
For an approved scope, the future workflow evaluates **Authoritative and Recursive DNS** through
...
| --- | --- |
| Owning platform | Authoritative and Recursive DNS turns a versioned platform intent into a repeatable decision rather than an isolated operator action. |
...
1. The request identifies **Authoritative and Recursive DNS**, the enterprise outcome, owner, immutable
...
```mermaid
flowchart LR
    Need["Enterprise need and owner"] --> Contract["Versioned Authoritative and Recursive DNS contract"]
...
These considerations make the page specific to **Authoritative and Recursive DNS** while preserving the
...
| Planned `UC-NET-005/contract` | Define inputs, owner, target allowlist, mode, coverage, policy, outputs, and safe stop |
| Planned `UC-NET-005/result-schema` | Normalize provenance, observed values, decision, safety, recovery, and review fields |
| Planned `UC-NET-005/fixtures` | Exercise passing, blocking, malformed, unauthorized, unavailable-dependency, and recovery cases |
...
**Description:** The platform owner and enterprise consumer need Authoritative and Recursive DNS defined
