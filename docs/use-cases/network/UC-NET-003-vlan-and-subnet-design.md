...
Network engineer, service owner, platform engineer, security reviewer, and SRE need a repeatable, auditable implementation of **VLAN and Subnet Design**.
The canonical coverage target is: Standard segmentation and routing domains. The work must strengthen the
...
evidence, and safe stop for VLAN and Subnet Design. CI proves source behavior with
...
| --- | --- |
| `use-cases/UC-NET-003/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-NET-003/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-NET-003/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-NET-003/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-NET-003.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-NET-003-001: Define and validate the platform contract

**Description:** The platform owner needs VLAN and Subnet Design expressed as versioned
...
sensitive inputs are rejected, and the expected coverage is: Standard segmentation and routing domains.
...
**Required attachments:** `ART-NET-003-001A` contract validation
and fixture report.

### STORY-NET-003-002: Execute safely and prove the outcome

**Description:** Platform and enterprise owners need a reproducible result for
VLAN and Subnet Design that distinguishes source completion from runtime acceptance and
...
**Required attachments:** `ART-NET-003-002A` execution result,
`ART-NET-003-002B` rollback or non-mutation proof, and
`ATT-NET-003-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-NET-003-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-NET-003-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-NET-003-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-NET-003-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Standard segmentation and routing domains | Defined in canonical portfolio |
