# UC-RSO-006: Automated Incident Evidence Collection
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | Automated Incident Evidence Collection |
| Primary platform | Enterprise Resilience and Service Operations Platform |
| Supporting use cases | [UC-RSO-004](UC-RSO-004-incident-detection-and-classification.md), [UC-RSO-005](UC-RSO-005-on-call-and-escalation-workflows.md), [UC-OBS-014](../observability/UC-OBS-014-change-to-incident-correlation.md), [UC-OBS-001](../observability/UC-OBS-001-slo-as-code.md) |
...
| Jira epic | `EPIC-RSO-006` — Implement Automated Incident Evidence Collection |
...
Automated Incident Evidence Collection makes **Logs, metrics, traces and changes captured** an owned and rehearsable resilience outcome instead of a runbook assumption.

For Automated Incident Evidence Collection, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Logs, metrics, traces and changes captured** on the documented
...
Acceptance for Automated Incident Evidence Collection requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Resilience and Service Operations Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Automated Incident Evidence Collection. |
...
- The implementation contract defines the scope required to demonstrate:
  **Logs, metrics, traces and changes captured**
...
- the versioned contract for Automated Incident Evidence Collection;
...
A useful way for a new engineer to understand Automated Incident Evidence Collection is to begin
with user impact and decision authority, then connect diagnosis, mitigation and verified
recovery. The result MidhHealth needs is to connect service ownership, evidence, incident
response, and recovery for enterprise workflows. Enterprise Resilience and Service Operations
...
escalation acknowledgement path. The first buildable boundary is existing GitLab, Jenkins, AWX,
...
Automated Incident Evidence Collection is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Logs, metrics, traces and changes captured**—rather than a new product or
environment.
...
Automated Incident Evidence Collection. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-RSO-006 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-RSO-006/UC-RSO-006-architecture.svg)
...
Automated Incident Evidence Collection remains accountable to its primary platform. The dependencies below
...
Before Automated Incident Evidence Collection is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For Automated Incident Evidence Collection, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Logs, metrics, traces and changes captured** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Automated Incident Evidence Collection become
...
The Automated Incident Evidence Collection design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Automated Incident Evidence Collection | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Automated Incident Evidence Collection, the primary threat is **an exercise expanding beyond its approved service, dependency, or operator boundary**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-RSO-006 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Automated Incident Evidence Collection implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/reliability-operations/resilience-service-operations/contracts/uc-rso-006.yaml` |
| Primary implementation | `midhhealth/reliability-operations/resilience-service-operations/playbooks/automated-incident-evidence-collection.yml`; entry point: the `automated-incident-evidence-collection` readiness check or bounded exercise |
| Machine-readable result schema | `midhhealth/reliability-operations/resilience-service-operations/schemas/uc-rso-006-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/reliability-operations/resilience-service-operations/tests/fixtures/uc-rso-006/` |
| GitLab source gate | `midhhealth/reliability-operations/resilience-service-operations/.gitlab/ci/uc-rso-006.yml` |
| Operator diagnosis and recovery | `midhhealth/reliability-operations/resilience-service-operations/docs/runbooks/uc-rso-006.md` |
...
3. **Read-only proof:** execute the `automated-incident-evidence-collection` readiness check or bounded exercise, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/reliability-operations/resilience-service-operations/contracts/uc-rso-006.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/reliability-operations/resilience-service-operations/playbooks/automated-incident-evidence-collection.yml` | Primary implementation through the `automated-incident-evidence-collection` readiness check or bounded exercise | Planned |
| `midhhealth/reliability-operations/resilience-service-operations/schemas/uc-rso-006-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/reliability-operations/resilience-service-operations/tests/fixtures/uc-rso-006/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/reliability-operations/resilience-service-operations/.gitlab/ci/uc-rso-006.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/reliability-operations/resilience-service-operations/docs/runbooks/uc-rso-006.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-RSO-006-001: Define the Automated Incident Evidence Collection contract
...
**Implementation steps:** Write `midhhealth/reliability-operations/resilience-service-operations/docs/runbooks/uc-rso-006.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-RSO-006-001A` contract and fixture review.

### STORY-RSO-006-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Automated Incident Evidence Collection, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-RSO-006-002A` source validation and
`ART-RSO-006-002B` downstream-block proof.

### STORY-RSO-006-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Logs, metrics, traces and changes captured** on its named scope without hidden
...
**Required attachments:** Future `ART-RSO-006-003A` bounded result,
`ART-RSO-006-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-RSO-006-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-RSO-006-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-RSO-006-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-RSO-006-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-RSO-006-003A` | Bounded observed result compared with **Logs, metrics, traces and changes captured** | Approved existing execution path | Pending future execution |
| `ART-RSO-006-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Logs, metrics, traces and changes captured | Detailed behavior and decision semantics documented |
