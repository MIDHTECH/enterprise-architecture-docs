...
Service owner, SRE, incident responder, platform owner, and governance reviewer need a repeatable, auditable implementation of **Automated Incident Evidence Collection**.
The canonical coverage target is: Logs, metrics, traces and changes captured. The work must strengthen the
...
evidence, and safe stop for Automated Incident Evidence Collection. CI proves source behavior with
...
| Primary actors | Service owner, SRE, incident responder, platform owner, and governance reviewer |
...
- Execution uses existing GitLab, Jenkins, AWX, observability APIs, service records, and runbooks.
...
rollback or safe stop. The page does not authorize a new service-management product, VM, monitoring stack, or unapproved disruptive exercise. Any new
...
| --- | --- |
| `use-cases/UC-RSO-006/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-RSO-006/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-RSO-006/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-RSO-006/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-RSO-006.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-RSO-006-001: Define and validate the platform contract

**Description:** The platform owner needs Automated Incident Evidence Collection expressed as versioned
...
sensitive inputs are rejected, and the expected coverage is: Logs, metrics, traces and changes captured.
...
**Required attachments:** `ART-RSO-006-001A` contract validation
and fixture report.

### STORY-RSO-006-002: Execute safely and prove the outcome

**Description:** Platform and enterprise owners need a reproducible result for
Automated Incident Evidence Collection that distinguishes source completion from runtime acceptance and
...
**Required attachments:** `ART-RSO-006-002A` execution result,
`ART-RSO-006-002B` rollback or non-mutation proof, and
`ATT-RSO-006-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-RSO-006-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-RSO-006-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-RSO-006-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-RSO-006-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Logs, metrics, traces and changes captured | Defined in canonical portfolio |
| Enterprise fit | connect service ownership, evidence, incident response, and recovery for enterprise workflows | Traceability specified; owner review pending |
...
- Copy this specification into `midhhealth/reliability-operations/resilience-service-operations` as the epic and story contract.
