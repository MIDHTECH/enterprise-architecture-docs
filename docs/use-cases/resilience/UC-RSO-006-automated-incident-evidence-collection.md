...
**Automated Incident Evidence Collection** addresses a specific operating need inside the
Enterprise Resilience and Service Operations Platform: **Logs, metrics, traces and changes captured**. Without a shared design, teams can perform
...
The design connects service-resilience objective, operating signal, and named existing service scope to a controlled result. It gives
service owner, SRE, incident commander, operations engineer, and risk reviewer a common description of the trigger, inputs, boundaries,
...
For an approved scope, the future workflow evaluates **Automated Incident Evidence Collection** through
read-only readiness or failure analysis followed by an approved bounded drill or remediation through existing automation when applicable. It produces a deterministic allow, block, escalate, or
not-applicable decision tied to immutable source and the named target. The
decision supports incident, continuity, recovery, capacity, change, and service-acceptance decisions and advances continuity and controlled recovery of provider, payer, and shared-platform services.
...
| --- | --- |
| Owning platform | Automated Incident Evidence Collection turns a versioned platform intent into a repeatable decision rather than an isolated operator action. |
| Platform workflow | drills and automation use existing services and control planes; no standby environment, monitoring product, or recovery capacity is created. |
| Enterprise outcome | The result contributes to connect service ownership, evidence, incident response, and recovery for enterprise workflows and remains traceable to its owner and source. |
...
1. The request identifies **Automated Incident Evidence Collection**, the enterprise outcome, owner, immutable
...
6. If mutation is necessary, the owner obtains the required change approval and
   ...
   non-mutating.
7. The result records service and scenario, run ID, timeline, observed impact, objective result, decision, recovery proof, and incident/action links and explains why the coverage was or
   was not satisfied.
...
```mermaid
flowchart LR
    Need["Enterprise need and owner"] --> Contract["Versioned Automated Incident Evidence Collection contract"]
...
These considerations make the page specific to **Automated Incident Evidence Collection** while preserving the
same enterprise control language used across the owning platform. In particular,
the later implementation must demonstrate signal quality, telemetry integrity, service continuity.
...
| --- | --- |
| `midhhealth/reliability-operations/resilience-service-operations` | Own the future platform implementation and use-case-specific operating notes |
| Planned `UC-RSO-006/contract` | Define inputs, owner, target allowlist, mode, coverage, policy, outputs, and safe stop |
| Planned `UC-RSO-006/result-schema` | Normalize provenance, observed values, decision, safety, recovery, and review fields |
| Planned `UC-RSO-006/fixtures` | Exercise passing, blocking, malformed, unauthorized, unavailable-dependency, and recovery cases |
...
**Description:** The platform owner and enterprise consumer need Automated Incident Evidence Collection defined
