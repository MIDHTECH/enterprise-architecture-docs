...
**Maintenance-Window Management** addresses a specific operating need inside the
Enterprise Resilience and Service Operations Platform: **Planned service-impact coordination**. Without a shared design, teams can perform
...
For an approved scope, the future workflow evaluates **Maintenance-Window Management** through
...
| --- | --- |
| Owning platform | Maintenance-Window Management turns a versioned platform intent into a repeatable decision rather than an isolated operator action. |
...
1. The request identifies **Maintenance-Window Management**, the enterprise outcome, owner, immutable
...
```mermaid
flowchart LR
    Need["Enterprise need and owner"] --> Contract["Versioned Maintenance-Window Management contract"]
...
These considerations make the page specific to **Maintenance-Window Management** while preserving the
...
| Planned `UC-RSO-020/contract` | Define inputs, owner, target allowlist, mode, coverage, policy, outputs, and safe stop |
| Planned `UC-RSO-020/result-schema` | Normalize provenance, observed values, decision, safety, recovery, and review fields |
| Planned `UC-RSO-020/fixtures` | Exercise passing, blocking, malformed, unauthorized, unavailable-dependency, and recovery cases |
...
**Description:** The platform owner and enterprise consumer need Maintenance-Window Management defined
