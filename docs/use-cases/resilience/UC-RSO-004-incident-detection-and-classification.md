# UC-RSO-004: Incident Detection and Classification
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | Incident Detection and Classification |
| Primary platform | Enterprise Resilience and Service Operations Platform |
| Supporting use cases | [UC-RSO-005](UC-RSO-005-on-call-and-escalation-workflows.md), [UC-OBS-014](../observability/UC-OBS-014-change-to-incident-correlation.md), [UC-OBS-001](../observability/UC-OBS-001-slo-as-code.md), [UC-RSO-009](UC-RSO-009-service-ownership.md) |
...
| Jira epic | `EPIC-RSO-004` — Implement Incident Detection and Classification |
...
Incident Detection and Classification makes **Consistent severity and ownership** an owned and rehearsable resilience outcome instead of a runbook assumption.

For Incident Detection and Classification, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Consistent severity and ownership** on the documented
...
Acceptance for Incident Detection and Classification requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Resilience and Service Operations Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Incident Detection and Classification. |
...
- The implementation contract defines the scope required to demonstrate:
  **Consistent severity and ownership**
...
- the versioned contract for Incident Detection and Classification;
...
Rather than beginning with a product, explain Incident Detection and Classification by asking an
engineer to begin with user impact and decision authority, then connect diagnosis, mitigation
and verified recovery. The result MidhHealth needs is to connect service ownership, evidence,
incident response, and recovery for enterprise workflows. Enterprise Resilience and Service
Operations Platform team owns the platform decision, while the consuming service or business
...
verify recovery and preserve what the team learned. In this page, **UC-RSO-005: On-Call and
Escalation Workflows** contributes on-call owner and escalation acknowledgement path;
**UC-OBS-014: Change-to-Incident Correlation** contributes change identity linked to incident
timing and symptoms. The first buildable boundary is existing GitLab, Jenkins, AWX,
...
Incident Detection and Classification is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Consistent severity and ownership**—rather than a new product or
environment.
...
Incident Detection and Classification. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-RSO-004 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-RSO-004/UC-RSO-004-architecture.svg)
...
Incident Detection and Classification remains accountable to its primary platform. The dependencies below
provide explicit contracts or assurance evidence; they do not become alternate owners.

| Relationship | Use case | Required handoff | Failure propagation |
| --- | --- | --- | --- |
| Required upstream contract | [UC-RSO-005: On-Call and Escalation Workflows](UC-RSO-005-on-call-and-escalation-workflows.md) | on-call owner and escalation acknowledgement path | Missing, stale, or failed evidence blocks promotion or runtime action. |
...
Before Incident Detection and Classification is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For Incident Detection and Classification, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Consistent severity and ownership** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Incident Detection and Classification become
...
The Incident Detection and Classification design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Incident Detection and Classification | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Incident Detection and Classification, the primary threat is **an exercise expanding beyond its approved service, dependency, or operator boundary**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-RSO-004 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Incident Detection and Classification implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/reliability-operations/resilience-service-operations/contracts/uc-rso-004.yaml` |
| Primary implementation | `midhhealth/reliability-operations/resilience-service-operations/playbooks/incident-detection-and-classification.yml`; entry point: the `incident-detection-and-classification` readiness check or bounded exercise |
| Machine-readable result schema | `midhhealth/reliability-operations/resilience-service-operations/schemas/uc-rso-004-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/reliability-operations/resilience-service-operations/tests/fixtures/uc-rso-004/` |
| GitLab source gate | `midhhealth/reliability-operations/resilience-service-operations/.gitlab/ci/uc-rso-004.yml` |
| Operator diagnosis and recovery | `midhhealth/reliability-operations/resilience-service-operations/docs/runbooks/uc-rso-004.md` |
...
3. **Read-only proof:** execute the `incident-detection-and-classification` readiness check or bounded exercise, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/reliability-operations/resilience-service-operations/contracts/uc-rso-004.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/reliability-operations/resilience-service-operations/playbooks/incident-detection-and-classification.yml` | Primary implementation through the `incident-detection-and-classification` readiness check or bounded exercise | Planned |
| `midhhealth/reliability-operations/resilience-service-operations/schemas/uc-rso-004-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/reliability-operations/resilience-service-operations/tests/fixtures/uc-rso-004/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/reliability-operations/resilience-service-operations/.gitlab/ci/uc-rso-004.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/reliability-operations/resilience-service-operations/docs/runbooks/uc-rso-004.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-RSO-004-001: Define the Incident Detection and Classification contract
...
**Implementation steps:** Write `midhhealth/reliability-operations/resilience-service-operations/docs/runbooks/uc-rso-004.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-RSO-004-001A` contract and fixture review.

### STORY-RSO-004-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Incident Detection and Classification, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-RSO-004-002A` source validation and
`ART-RSO-004-002B` downstream-block proof.

### STORY-RSO-004-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Consistent severity and ownership** on its named scope without hidden
...
**Required attachments:** Future `ART-RSO-004-003A` bounded result,
`ART-RSO-004-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-RSO-004-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-RSO-004-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-RSO-004-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-RSO-004-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-RSO-004-003A` | Bounded observed result compared with **Consistent severity and ownership** | Approved existing execution path | Pending future execution |
| `ART-RSO-004-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Consistent severity and ownership | Detailed behavior and decision semantics documented |
