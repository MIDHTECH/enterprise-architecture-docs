...
Service owner, SRE, incident responder, platform owner, and governance reviewer need a repeatable, auditable implementation of **RTO and RPO Measurement**.
The canonical coverage target is: Evidence-based recovery objectives. The work must strengthen the
...
evidence, and safe stop for RTO and RPO Measurement. CI proves source behavior with
...
| --- | --- |
| `use-cases/UC-RSO-017/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-RSO-017/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-RSO-017/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-RSO-017/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-RSO-017.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-RSO-017-001: Define and validate the platform contract

**Description:** The platform owner needs RTO and RPO Measurement expressed as versioned
...
sensitive inputs are rejected, and the expected coverage is: Evidence-based recovery objectives.
...
**Required attachments:** `ART-RSO-017-001A` contract validation
and fixture report.

### STORY-RSO-017-002: Execute safely and prove the outcome

**Description:** Platform and enterprise owners need a reproducible result for
RTO and RPO Measurement that distinguishes source completion from runtime acceptance and
...
**Required attachments:** `ART-RSO-017-002A` execution result,
`ART-RSO-017-002B` rollback or non-mutation proof, and
`ATT-RSO-017-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-RSO-017-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-RSO-017-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-RSO-017-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-RSO-017-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Evidence-based recovery objectives | Defined in canonical portfolio |
