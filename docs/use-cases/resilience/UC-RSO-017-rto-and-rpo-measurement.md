# UC-RSO-017: RTO and RPO Measurement
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | RTO and RPO Measurement |
| Primary platform | Enterprise Resilience and Service Operations Platform |
| Supporting use cases | [UC-RSO-015](UC-RSO-015-backup-and-recovery-orchestration.md), [UC-DB-001](../database/UC-DB-001-backup-restore-validation.md), [UC-OBS-001](../observability/UC-OBS-001-slo-as-code.md), [UC-RSO-009](UC-RSO-009-service-ownership.md) |
...
| Jira epic | `EPIC-RSO-017` — Implement RTO and RPO Measurement |
...
RTO and RPO Measurement makes **Evidence-based recovery objectives** an owned and rehearsable resilience outcome instead of a runbook assumption.

For RTO and RPO Measurement, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Evidence-based recovery objectives** on the documented
...
Acceptance for RTO and RPO Measurement requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Resilience and Service Operations Platform** owns the contract, control behavior, evidence schema, and recovery boundary for RTO and RPO Measurement. |
...
- The implementation contract defines the scope required to demonstrate:
  **Evidence-based recovery objectives**
...
- the versioned contract for RTO and RPO Measurement;
...
In practice, RTO and RPO Measurement makes sense when you begin with user impact and decision
...
identity or evidence from an earlier one. In this page, **UC-RSO-015: Backup and Recovery
Orchestration** contributes readiness or exercise result tied to observed service recovery;
**UC-DB-001: Automated PostgreSQL Restore Validation** contributes backup provenance, isolated
restore, and recovery verification. The first buildable boundary is existing GitLab, Jenkins,
AWX, observability APIs, service records, and runbooks. The design stops at this rule: Reuse the
...
RTO and RPO Measurement is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Evidence-based recovery objectives**—rather than a new product or
environment.
...
RTO and RPO Measurement. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-RSO-017 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-RSO-017/UC-RSO-017-architecture.svg)
...
RTO and RPO Measurement remains accountable to its primary platform. The dependencies below
provide explicit contracts or assurance evidence; they do not become alternate owners.

| Relationship | Use case | Required handoff | Failure propagation |
| --- | --- | --- | --- |
| Required upstream contract | [UC-RSO-015: Backup and Recovery Orchestration](UC-RSO-015-backup-and-recovery-orchestration.md) | readiness or exercise result tied to observed service recovery | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |
| Required upstream contract | [UC-DB-001: Automated PostgreSQL Restore Validation](../database/UC-DB-001-backup-restore-validation.md) | backup provenance, isolated restore, and recovery verification | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |
...
| Coordinated assurance handoff | [UC-RSO-009: Service Ownership](UC-RSO-009-service-ownership.md) | readiness or exercise result tied to observed service recovery | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |

Before RTO and RPO Measurement is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For RTO and RPO Measurement, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Evidence-based recovery objectives** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for RTO and RPO Measurement become
...
The RTO and RPO Measurement design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for RTO and RPO Measurement | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For RTO and RPO Measurement, the primary threat is **an exercise expanding beyond its approved service, dependency, or operator boundary**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-RSO-017 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first RTO and RPO Measurement implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/reliability-operations/resilience-service-operations/contracts/uc-rso-017.yaml` |
| Primary implementation | `midhhealth/reliability-operations/resilience-service-operations/playbooks/rto-and-rpo-measurement.yml`; entry point: the `rto-and-rpo-measurement` readiness check or bounded exercise |
| Machine-readable result schema | `midhhealth/reliability-operations/resilience-service-operations/schemas/uc-rso-017-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/reliability-operations/resilience-service-operations/tests/fixtures/uc-rso-017/` |
| GitLab source gate | `midhhealth/reliability-operations/resilience-service-operations/.gitlab/ci/uc-rso-017.yml` |
| Operator diagnosis and recovery | `midhhealth/reliability-operations/resilience-service-operations/docs/runbooks/uc-rso-017.md` |
...
3. **Read-only proof:** execute the `rto-and-rpo-measurement` readiness check or bounded exercise, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/reliability-operations/resilience-service-operations/contracts/uc-rso-017.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/reliability-operations/resilience-service-operations/playbooks/rto-and-rpo-measurement.yml` | Primary implementation through the `rto-and-rpo-measurement` readiness check or bounded exercise | Planned |
| `midhhealth/reliability-operations/resilience-service-operations/schemas/uc-rso-017-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/reliability-operations/resilience-service-operations/tests/fixtures/uc-rso-017/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/reliability-operations/resilience-service-operations/.gitlab/ci/uc-rso-017.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/reliability-operations/resilience-service-operations/docs/runbooks/uc-rso-017.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-RSO-017-001: Define the RTO and RPO Measurement contract
...
**Implementation steps:** Write `midhhealth/reliability-operations/resilience-service-operations/docs/runbooks/uc-rso-017.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-RSO-017-001A` contract and fixture review.

### STORY-RSO-017-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates RTO and RPO Measurement, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-RSO-017-002A` source validation and
`ART-RSO-017-002B` downstream-block proof.

### STORY-RSO-017-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Evidence-based recovery objectives** on its named scope without hidden
...
**Required attachments:** Future `ART-RSO-017-003A` bounded result,
`ART-RSO-017-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-RSO-017-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-RSO-017-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-RSO-017-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-RSO-017-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-RSO-017-003A` | Bounded observed result compared with **Evidence-based recovery objectives** | Approved existing execution path | Pending future execution |
| `ART-RSO-017-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Evidence-based recovery objectives | Detailed behavior and decision semantics documented |
