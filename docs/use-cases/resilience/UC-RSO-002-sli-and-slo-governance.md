...
**SLI and SLO Governance** addresses a specific operating need inside the
Enterprise Resilience and Service Operations Platform: **Standard service-level objectives**. Without a shared design, teams can perform
...
For an approved scope, the future workflow evaluates **SLI and SLO Governance** through
...
| --- | --- |
| Owning platform | SLI and SLO Governance turns a versioned platform intent into a repeatable decision rather than an isolated operator action. |
...
1. The request identifies **SLI and SLO Governance**, the enterprise outcome, owner, immutable
...
```mermaid
flowchart LR
    Need["Enterprise need and owner"] --> Contract["Versioned SLI and SLO Governance contract"]
...
These considerations make the page specific to **SLI and SLO Governance** while preserving the
...
| Planned `UC-RSO-002/contract` | Define inputs, owner, target allowlist, mode, coverage, policy, outputs, and safe stop |
| Planned `UC-RSO-002/result-schema` | Normalize provenance, observed values, decision, safety, recovery, and review fields |
| Planned `UC-RSO-002/fixtures` | Exercise passing, blocking, malformed, unauthorized, unavailable-dependency, and recovery cases |
...
**Description:** The platform owner and enterprise consumer need SLI and SLO Governance defined
