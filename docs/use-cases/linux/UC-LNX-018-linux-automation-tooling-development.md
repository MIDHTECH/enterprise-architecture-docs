...
This use case implements **Linux Automation and Tooling Development** as a traceable IaC capability. Its operational trigger is: a repeated operational task needs a reusable, testable interface instead of copy-paste commands. Every outcome must be reproducible from reviewed source, bounded during execution, observable, idempotent, recoverable, and evidenced.

## Expected outcome

Tested Bash/Python utilities, APIs and reusable operational automation. Operators can trace the request to an immutable Git revision, review PLAN/CHECK, execute one canary, expand only through health gates, prove repeat convergence, recover through a tested path, and hand off evidence without relying on console memory.
...
**Description:** As a Linux platform engineer, I need variables, roles/modules, tests, pipeline gates, and an operating contract for Linux Automation and Tooling Development so runtime work does not depend on undocumented console state.
...
**Description:** As an operator, I need an approved PLAN/CHECK and canary-first APPLY for Linux Automation and Tooling Development so unsafe behavior stops before fleet expansion.
...
**Description:** As an SRE, I need repeat convergence, runtime health, recovery proof, and an evidence package for Linux Automation and Tooling Development so the capability is supportable and auditable.
