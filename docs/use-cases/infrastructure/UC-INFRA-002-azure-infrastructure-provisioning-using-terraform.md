...
Azure Infrastructure Provisioning Using Terraform governs **Azure stack covers resource group, VNet, VM, storage, database, container platform** against known infrastructure intent and inventory, keeping analysis separate from approved mutation.

For Azure Infrastructure Provisioning Using Terraform, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Azure stack covers resource group, VNet, VM, storage, database, container platform** on the documented
...
Acceptance for Azure Infrastructure Provisioning Using Terraform requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Multi-Cloud Infrastructure Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Azure Infrastructure Provisioning Using Terraform. |
...
- The implementation contract defines the scope required to demonstrate:
  **Azure stack covers resource group, VNet, VM, storage, database, container platform**
...
- the versioned contract for Azure Infrastructure Provisioning Using Terraform;
...
A useful way for a new engineer to understand Azure Infrastructure Provisioning Using Terraform
is to follow declared intent through plan, state ownership and post-change verification. The
result MidhHealth needs is to keep the existing lab foundation repeatable, attributable, and
...
Azure Infrastructure Provisioning Using Terraform is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Azure stack covers resource group, VNet, VM, storage, database, container platform**—rather than a new product or
environment.
...
Azure Infrastructure Provisioning Using Terraform. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-INFRA-002 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-INFRA-002/UC-INFRA-002-architecture.svg)
...
Azure Infrastructure Provisioning Using Terraform remains accountable to its primary platform. The dependencies below
...
Before Azure Infrastructure Provisioning Using Terraform is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For Azure Infrastructure Provisioning Using Terraform, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Azure stack covers resource group, VNet, VM, storage, database, container platform** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Azure Infrastructure Provisioning Using Terraform become
...
The Azure Infrastructure Provisioning Using Terraform design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Azure Infrastructure Provisioning Using Terraform | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Azure Infrastructure Provisioning Using Terraform, the primary threat is **over-privileged automation or an incorrect state/target selection**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-INFRA-002 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Azure Infrastructure Provisioning Using Terraform implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/platform-engineering/cloud-infra-automation-platform/contracts/uc-infra-002.yaml` |
| Primary implementation | `midhhealth/platform-engineering/cloud-infra-automation-platform/automation/azure-infrastructure-provisioning-using-terraform/main.yml`; entry point: the `azure-infrastructure-provisioning-using-terraform` plan and bounded execution entry point |
| Machine-readable result schema | `midhhealth/platform-engineering/cloud-infra-automation-platform/schemas/uc-infra-002-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/platform-engineering/cloud-infra-automation-platform/tests/fixtures/uc-infra-002/` |
| GitLab source gate | `midhhealth/platform-engineering/cloud-infra-automation-platform/.gitlab/ci/uc-infra-002.yml` |
| Operator diagnosis and recovery | `midhhealth/platform-engineering/cloud-infra-automation-platform/docs/runbooks/uc-infra-002.md` |
...
3. **Read-only proof:** execute the `azure-infrastructure-provisioning-using-terraform` plan and bounded execution entry point, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/contracts/uc-infra-002.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/automation/azure-infrastructure-provisioning-using-terraform/main.yml` | Primary implementation through the `azure-infrastructure-provisioning-using-terraform` plan and bounded execution entry point | Planned |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/schemas/uc-infra-002-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/tests/fixtures/uc-infra-002/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/.gitlab/ci/uc-infra-002.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/docs/runbooks/uc-infra-002.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-INFRA-002-001: Define the Azure Infrastructure Provisioning Using Terraform contract
...
**Implementation steps:** Write `midhhealth/platform-engineering/cloud-infra-automation-platform/docs/runbooks/uc-infra-002.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-INFRA-002-001A` contract and fixture review.

### STORY-INFRA-002-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Azure Infrastructure Provisioning Using Terraform, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-INFRA-002-002A` source validation and
`ART-INFRA-002-002B` downstream-block proof.

### STORY-INFRA-002-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Azure stack covers resource group, VNet, VM, storage, database, container platform** on its named scope without hidden
...
**Required attachments:** Future `ART-INFRA-002-003A` bounded result,
`ART-INFRA-002-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-INFRA-002-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-INFRA-002-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-INFRA-002-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-INFRA-002-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-INFRA-002-003A` | Bounded observed result compared with **Azure stack covers resource group, VNet, VM, storage, database, container platform** | Approved existing execution path | Pending future execution |
| `ART-INFRA-002-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Azure stack covers resource group, VNet, VM, storage, database, container platform | Detailed behavior and decision semantics documented |
