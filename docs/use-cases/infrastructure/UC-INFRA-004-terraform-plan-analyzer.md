# UC-INFRA-004: Terraform Plan Analyzer
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | Terraform Plan Analyzer |
...
| Jira epic | `EPIC-INFRA-004` — Implement Terraform Plan Analyzer |
...
Terraform Plan Analyzer governs **Plan output summarizes creates, updates, destroys and replacement risk** against known infrastructure intent and inventory, keeping analysis separate from approved mutation.

For Terraform Plan Analyzer, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Plan output summarizes creates, updates, destroys and replacement risk** on the documented
...
Acceptance for Terraform Plan Analyzer requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Multi-Cloud Infrastructure Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Terraform Plan Analyzer. |
...
- The implementation contract defines the scope required to demonstrate:
  **Plan output summarizes creates, updates, destroys and replacement risk**
...
- the versioned contract for Terraform Plan Analyzer;
...
The architecture conversation for Terraform Plan Analyzer should follow declared intent through
...
Terraform Plan Analyzer is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Plan output summarizes creates, updates, destroys and replacement risk**—rather than a new product or
environment.
...
Terraform Plan Analyzer. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-INFRA-004 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-INFRA-004/UC-INFRA-004-architecture.svg)
...
Terraform Plan Analyzer remains accountable to its primary platform. The dependencies below
...
Before Terraform Plan Analyzer is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For Terraform Plan Analyzer, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Plan output summarizes creates, updates, destroys and replacement risk** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Terraform Plan Analyzer become
...
The Terraform Plan Analyzer design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Terraform Plan Analyzer | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Terraform Plan Analyzer, the primary threat is **over-privileged automation or an incorrect state/target selection**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-INFRA-004 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Terraform Plan Analyzer implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/platform-engineering/cloud-infra-automation-platform/contracts/uc-infra-004.yaml` |
| Primary implementation | `midhhealth/platform-engineering/cloud-infra-automation-platform/automation/terraform-plan-analyzer/main.yml`; entry point: the `terraform-plan-analyzer` plan and bounded execution entry point |
| Machine-readable result schema | `midhhealth/platform-engineering/cloud-infra-automation-platform/schemas/uc-infra-004-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/platform-engineering/cloud-infra-automation-platform/tests/fixtures/uc-infra-004/` |
| GitLab source gate | `midhhealth/platform-engineering/cloud-infra-automation-platform/.gitlab/ci/uc-infra-004.yml` |
| Operator diagnosis and recovery | `midhhealth/platform-engineering/cloud-infra-automation-platform/docs/runbooks/uc-infra-004.md` |
...
3. **Read-only proof:** execute the `terraform-plan-analyzer` plan and bounded execution entry point, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/contracts/uc-infra-004.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/automation/terraform-plan-analyzer/main.yml` | Primary implementation through the `terraform-plan-analyzer` plan and bounded execution entry point | Planned |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/schemas/uc-infra-004-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/tests/fixtures/uc-infra-004/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/.gitlab/ci/uc-infra-004.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/docs/runbooks/uc-infra-004.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-INFRA-004-001: Define the Terraform Plan Analyzer contract
...
**Implementation steps:** Write `midhhealth/platform-engineering/cloud-infra-automation-platform/docs/runbooks/uc-infra-004.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-INFRA-004-001A` contract and fixture review.

### STORY-INFRA-004-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Terraform Plan Analyzer, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-INFRA-004-002A` source validation and
`ART-INFRA-004-002B` downstream-block proof.

### STORY-INFRA-004-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Plan output summarizes creates, updates, destroys and replacement risk** on its named scope without hidden
...
**Required attachments:** Future `ART-INFRA-004-003A` bounded result,
`ART-INFRA-004-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-INFRA-004-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-INFRA-004-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-INFRA-004-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-INFRA-004-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-INFRA-004-003A` | Bounded observed result compared with **Plan output summarizes creates, updates, destroys and replacement risk** | Approved existing execution path | Pending future execution |
| `ART-INFRA-004-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Plan output summarizes creates, updates, destroys and replacement risk | Detailed behavior and decision semantics documented |
