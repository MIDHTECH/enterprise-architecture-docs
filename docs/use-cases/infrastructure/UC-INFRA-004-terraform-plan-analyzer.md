...
**Terraform Plan Analyzer** addresses a specific operating need inside the
Enterprise Multi-Cloud Infrastructure Platform: **Plan output summarizes creates, updates, destroys and replacement risk**. Without a shared design, teams can perform
...
For an approved scope, the future workflow evaluates **Terraform Plan Analyzer** through
...
| --- | --- |
| Owning platform | Terraform Plan Analyzer turns a versioned platform intent into a repeatable decision rather than an isolated operator action. |
...
1. The request identifies **Terraform Plan Analyzer**, the enterprise outcome, owner, immutable
...
```mermaid
flowchart LR
    Need["Enterprise need and owner"] --> Contract["Versioned Terraform Plan Analyzer contract"]
...
These considerations make the page specific to **Terraform Plan Analyzer** while preserving the
...
| Planned `UC-INFRA-004/contract` | Define inputs, owner, target allowlist, mode, coverage, policy, outputs, and safe stop |
| Planned `UC-INFRA-004/result-schema` | Normalize provenance, observed values, decision, safety, recovery, and review fields |
| Planned `UC-INFRA-004/fixtures` | Exercise passing, blocking, malformed, unauthorized, unavailable-dependency, and recovery cases |
...
**Description:** The platform owner and enterprise consumer need Terraform Plan Analyzer defined
