...
Infrastructure engineer, service owner, governance reviewer, and SRE need a repeatable, auditable implementation of **Terraform Plan Analyzer**.
The canonical coverage target is: Plan output summarizes creates, updates, destroys and replacement risk. The work must strengthen the
...
evidence, and safe stop for Terraform Plan Analyzer. CI proves source behavior with
...
| --- | --- |
| `use-cases/UC-INFRA-004/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-INFRA-004/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-INFRA-004/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-INFRA-004/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-INFRA-004.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-INFRA-004-001: Define and validate the platform contract

**Description:** The platform owner needs Terraform Plan Analyzer expressed as versioned
...
sensitive inputs are rejected, and the expected coverage is: Plan output summarizes creates, updates, destroys and replacement risk.
...
**Required attachments:** `ART-INFRA-004-001A` contract validation
and fixture report.

### STORY-INFRA-004-002: Execute safely and prove the outcome

**Description:** Platform and enterprise owners need a reproducible result for
Terraform Plan Analyzer that distinguishes source completion from runtime acceptance and
...
**Required attachments:** `ART-INFRA-004-002A` execution result,
`ART-INFRA-004-002B` rollback or non-mutation proof, and
`ATT-INFRA-004-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-INFRA-004-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-INFRA-004-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-INFRA-004-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-INFRA-004-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Plan output summarizes creates, updates, destroys and replacement risk | Defined in canonical portfolio |
