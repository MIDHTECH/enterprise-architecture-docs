# UC-INFRA-006: Linux Server Patch Automation
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | Linux Server Patch Automation |
...
Linux Server Patch Automation governs **Ansible common role handles package baseline and can run patching** against known infrastructure intent and inventory, keeping analysis separate from approved mutation.

For Linux Server Patch Automation, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Ansible common role handles package baseline and can run patching** on the documented
...
Acceptance for Linux Server Patch Automation requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Multi-Cloud Infrastructure Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Linux Server Patch Automation. |
...
- The implementation contract defines the scope required to demonstrate:
  **Ansible common role handles package baseline and can run patching**
...
- the versioned contract for Linux Server Patch Automation;
...
For design review, walk through Linux Server Patch Automation by trying to follow declared
...
an owner and a recoverable prior state. In this page, **UC-LNX-006: Operating-System Patching**
contributes patch selection, canary health, cohort result, and deferral record; **UC-LNX-022:
Vulnerability Remediation Lifecycle** contributes affected-host, owner, remediation, exception,
and closure evidence. The first buildable boundary is existing GitLab infrastructure runner,
...
Linux Server Patch Automation is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Ansible common role handles package baseline and can run patching**—rather than a new product or
environment.
...
Linux Server Patch Automation. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-INFRA-006 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-INFRA-006/UC-INFRA-006-architecture.svg)
...
Linux Server Patch Automation remains accountable to its primary platform. The dependencies below
provide explicit contracts or assurance evidence; they do not become alternate owners.

| Relationship | Use case | Required handoff | Failure propagation |
| --- | --- | --- | --- |
| Required upstream contract | [UC-LNX-006: Operating-System Patching](../linux/UC-LNX-006-operating-system-patching.md) | patch selection, canary health, cohort result, and deferral record | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |
| Required upstream contract | [UC-LNX-022: Vulnerability Remediation Lifecycle](../linux/UC-LNX-022-vulnerability-remediation-lifecycle.md) | affected-host, owner, remediation, exception, and closure evidence | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |
| Coordinated assurance handoff | [UC-RSO-020: Maintenance-Window Management](../resilience/UC-RSO-020-maintenance-window-management.md) | readiness or exercise result tied to observed service recovery | Missing, stale, or contradictory handoff stops the dependent decision and is recorded for the accountable owner. |
...
Before Linux Server Patch Automation is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For Linux Server Patch Automation, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Ansible common role handles package baseline and can run patching** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Linux Server Patch Automation become
...
The Linux Server Patch Automation design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Linux Server Patch Automation | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Linux Server Patch Automation, the primary threat is **over-privileged automation or an incorrect state/target selection**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-INFRA-006 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Linux Server Patch Automation implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/platform-engineering/cloud-infra-automation-platform/contracts/uc-infra-006.yaml` |
| Primary implementation | `midhhealth/platform-engineering/cloud-infra-automation-platform/automation/linux-server-patch-automation/main.yml`; entry point: the `linux-server-patch-automation` plan and bounded execution entry point |
| Machine-readable result schema | `midhhealth/platform-engineering/cloud-infra-automation-platform/schemas/uc-infra-006-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/platform-engineering/cloud-infra-automation-platform/tests/fixtures/uc-infra-006/` |
| GitLab source gate | `midhhealth/platform-engineering/cloud-infra-automation-platform/.gitlab/ci/uc-infra-006.yml` |
| Operator diagnosis and recovery | `midhhealth/platform-engineering/cloud-infra-automation-platform/docs/runbooks/uc-infra-006.md` |
...
3. **Read-only proof:** execute the `linux-server-patch-automation` plan and bounded execution entry point, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/contracts/uc-infra-006.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/automation/linux-server-patch-automation/main.yml` | Primary implementation through the `linux-server-patch-automation` plan and bounded execution entry point | Planned |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/schemas/uc-infra-006-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/tests/fixtures/uc-infra-006/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/.gitlab/ci/uc-infra-006.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/platform-engineering/cloud-infra-automation-platform/docs/runbooks/uc-infra-006.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-INFRA-006-001: Define the Linux Server Patch Automation contract
...
**Implementation steps:** Write `midhhealth/platform-engineering/cloud-infra-automation-platform/docs/runbooks/uc-infra-006.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-INFRA-006-001A` contract and fixture review.

### STORY-INFRA-006-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Linux Server Patch Automation, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-INFRA-006-002A` source validation and
`ART-INFRA-006-002B` downstream-block proof.

### STORY-INFRA-006-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Ansible common role handles package baseline and can run patching** on its named scope without hidden
...
**Required attachments:** Future `ART-INFRA-006-003A` bounded result,
`ART-INFRA-006-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-INFRA-006-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-INFRA-006-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-INFRA-006-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-INFRA-006-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-INFRA-006-003A` | Bounded observed result compared with **Ansible common role handles package baseline and can run patching** | Approved existing execution path | Pending future execution |
| `ART-INFRA-006-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Ansible common role handles package baseline and can run patching | Detailed behavior and decision semantics documented |
