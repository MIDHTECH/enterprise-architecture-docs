...
**Data Classification** addresses a specific operating need inside the
Enterprise Data Engineering and Integration Platform: **Sensitivity and regulatory metadata**. Without a shared design, teams can perform
...
For an approved scope, the future workflow evaluates **Data Classification** through
...
| --- | --- |
| Owning platform | Data Classification turns a versioned platform intent into a repeatable decision rather than an isolated operator action. |
...
1. The request identifies **Data Classification**, the enterprise outcome, owner, immutable
...
```mermaid
flowchart LR
    Need["Enterprise need and owner"] --> Contract["Versioned Data Classification contract"]
...
These considerations make the page specific to **Data Classification** while preserving the
...
| Planned `UC-DATA-015/contract` | Define inputs, owner, target allowlist, mode, coverage, policy, outputs, and safe stop |
| Planned `UC-DATA-015/result-schema` | Normalize provenance, observed values, decision, safety, recovery, and review fields |
| Planned `UC-DATA-015/fixtures` | Exercise passing, blocking, malformed, unauthorized, unavailable-dependency, and recovery cases |
...
**Description:** The platform owner and enterprise consumer need Data Classification defined
