# UC-DATA-009: dbt Data Transformation
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | dbt Data Transformation |
...
| Jira epic | `EPIC-DATA-009` — Implement dbt Data Transformation |
...
dbt Data Transformation defines the data contract and control path for **Tested SQL transformation and documentation** before downstream provider or payer workflows consume the result.

For dbt Data Transformation, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Tested SQL transformation and documentation** on the documented
...
Acceptance for dbt Data Transformation requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise Data Engineering and Integration Platform** owns the contract, control behavior, evidence schema, and recovery boundary for dbt Data Transformation. |
...
- The implementation contract defines the scope required to demonstrate:
  **Tested SQL transformation and documentation**
...
- the versioned contract for dbt Data Transformation;
...
Rather than beginning with a product, explain dbt Data Transformation by asking an engineer to
follow a named producer record through validation, transformation and an accountable consumer.
The result MidhHealth needs is to move and validate healthcare data safely before downstream
enterprise decisions use it. Enterprise Data Engineering and Integration Platform team owns the
...
dbt Data Transformation is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Tested SQL transformation and documentation**—rather than a new product or
environment.
...
dbt Data Transformation. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-DATA-009 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-DATA-009/UC-DATA-009-architecture.svg)
...
dbt Data Transformation remains accountable to its primary platform. The dependencies below
...
Before dbt Data Transformation is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For dbt Data Transformation, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Tested SQL transformation and documentation** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for dbt Data Transformation become
...
The dbt Data Transformation design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for dbt Data Transformation | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For dbt Data Transformation, the primary threat is **protected or misclassified data entering fixtures, logs, or evidence**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-DATA-009 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first dbt Data Transformation implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/data-and-integration/data-engineering-platform/contracts/uc-data-009.yaml` |
| Primary implementation | `midhhealth/data-and-integration/data-engineering-platform/src/use_cases/dbt-data-transformation.py`; entry point: the `run_dbt_data_transformation` evaluation entry point |
| Machine-readable result schema | `midhhealth/data-and-integration/data-engineering-platform/schemas/uc-data-009-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/data-and-integration/data-engineering-platform/tests/fixtures/uc-data-009/` |
| GitLab source gate | `midhhealth/data-and-integration/data-engineering-platform/.gitlab/ci/uc-data-009.yml` |
| Operator diagnosis and recovery | `midhhealth/data-and-integration/data-engineering-platform/docs/runbooks/uc-data-009.md` |
...
3. **Read-only proof:** execute the `run_dbt_data_transformation` evaluation entry point, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/data-and-integration/data-engineering-platform/contracts/uc-data-009.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/data-and-integration/data-engineering-platform/src/use_cases/dbt-data-transformation.py` | Primary implementation through the `run_dbt_data_transformation` evaluation entry point | Planned |
| `midhhealth/data-and-integration/data-engineering-platform/schemas/uc-data-009-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/data-and-integration/data-engineering-platform/tests/fixtures/uc-data-009/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/data-and-integration/data-engineering-platform/.gitlab/ci/uc-data-009.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/data-and-integration/data-engineering-platform/docs/runbooks/uc-data-009.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-DATA-009-001: Define the dbt Data Transformation contract
...
**Implementation steps:** Write `midhhealth/data-and-integration/data-engineering-platform/docs/runbooks/uc-data-009.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-DATA-009-001A` contract and fixture review.

### STORY-DATA-009-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates dbt Data Transformation, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-DATA-009-002A` source validation and
`ART-DATA-009-002B` downstream-block proof.

### STORY-DATA-009-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Tested SQL transformation and documentation** on its named scope without hidden
...
**Required attachments:** Future `ART-DATA-009-003A` bounded result,
`ART-DATA-009-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-DATA-009-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-DATA-009-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-DATA-009-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-DATA-009-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-DATA-009-003A` | Bounded observed result compared with **Tested SQL transformation and documentation** | Approved existing execution path | Pending future execution |
| `ART-DATA-009-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Tested SQL transformation and documentation | Detailed behavior and decision semantics documented |
