...
**Data Reconciliation** addresses a specific operating need inside the
Enterprise Data Engineering and Integration Platform: **Source and target correctness validation**. Without a shared design, teams can perform
...
For an approved scope, the future workflow evaluates **Data Reconciliation** through
...
| --- | --- |
| Owning platform | Data Reconciliation turns a versioned platform intent into a repeatable decision rather than an isolated operator action. |
...
1. The request identifies **Data Reconciliation**, the enterprise outcome, owner, immutable
...
```mermaid
flowchart LR
    Need["Enterprise need and owner"] --> Contract["Versioned Data Reconciliation contract"]
...
These considerations make the page specific to **Data Reconciliation** while preserving the
same enterprise control language used across the owning platform. In particular,
the later implementation must demonstrate desired-versus-observed state, decision completeness, data trust.
...
| Planned `UC-DATA-021/contract` | Define inputs, owner, target allowlist, mode, coverage, policy, outputs, and safe stop |
| Planned `UC-DATA-021/result-schema` | Normalize provenance, observed values, decision, safety, recovery, and review fields |
| Planned `UC-DATA-021/fixtures` | Exercise passing, blocking, malformed, unauthorized, unavailable-dependency, and recovery cases |
...
**Description:** The platform owner and enterprise consumer need Data Reconciliation defined
