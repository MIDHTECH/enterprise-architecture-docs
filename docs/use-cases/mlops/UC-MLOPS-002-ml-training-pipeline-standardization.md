...
ML engineer, MLOps engineer, data owner, governance reviewer, workflow owner, and SRE need a repeatable, auditable implementation of **ML Training Pipeline Standardization**.
The canonical coverage target is: Repeatable training workflows. The work must strengthen the
...
evidence, and safe stop for ML Training Pipeline Standardization. CI proves source behavior with
...
| Primary actors | ML engineer, MLOps engineer, data owner, governance reviewer, workflow owner, and SRE |
...
- Execution uses existing GitLab shared runner, synthetic datasets, locked dependencies, and protected CI artifacts.
...
rollback or safe stop. The page does not authorize a new registry service, feature store, model server, VM, cluster workload, cloud ML service, or live-data scoring. Any new
...
| --- | --- |
| `use-cases/UC-MLOPS-002/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-MLOPS-002/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-MLOPS-002/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-MLOPS-002/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-MLOPS-002.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-MLOPS-002-001: Define and validate the platform contract

**Description:** The platform owner needs ML Training Pipeline Standardization expressed as versioned
...
sensitive inputs are rejected, and the expected coverage is: Repeatable training workflows.
...
**Required attachments:** `ART-MLOPS-002-001A` contract validation
and fixture report.

### STORY-MLOPS-002-002: Execute safely and prove the outcome

**Description:** Platform and enterprise owners need a reproducible result for
ML Training Pipeline Standardization that distinguishes source completion from runtime acceptance and
...
**Required attachments:** `ART-MLOPS-002-002A` execution result,
`ART-MLOPS-002-002B` rollback or non-mutation proof, and
`ATT-MLOPS-002-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-MLOPS-002-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-MLOPS-002-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-MLOPS-002-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-MLOPS-002-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Repeatable training workflows | Defined in canonical portfolio |
| Enterprise fit | make model lifecycle evidence reproducible before any model can affect an enterprise workflow | Traceability specified; owner review pending |
...
- Copy this specification into `midhhealth/ai-and-ml-platform/mlops-model-platform` as the epic and story contract.
