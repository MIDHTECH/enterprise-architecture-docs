...
ML engineer, MLOps engineer, data owner, governance reviewer, workflow owner, and SRE need a repeatable, auditable implementation of **Automated Retraining**.
The canonical coverage target is: Controlled retraining triggers and approvals. The work must strengthen the
...
evidence, and safe stop for Automated Retraining. CI proves source behavior with
...
| --- | --- |
| `use-cases/UC-MLOPS-010/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-MLOPS-010/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-MLOPS-010/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-MLOPS-010/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-MLOPS-010.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-MLOPS-010-001: Define and validate the platform contract

**Description:** The platform owner needs Automated Retraining expressed as versioned
...
sensitive inputs are rejected, and the expected coverage is: Controlled retraining triggers and approvals.
...
**Required attachments:** `ART-MLOPS-010-001A` contract validation
and fixture report.

### STORY-MLOPS-010-002: Execute safely and prove the outcome

**Description:** Platform and enterprise owners need a reproducible result for
Automated Retraining that distinguishes source completion from runtime acceptance and
...
**Required attachments:** `ART-MLOPS-010-002A` execution result,
`ART-MLOPS-010-002B` rollback or non-mutation proof, and
`ATT-MLOPS-010-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-MLOPS-010-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-MLOPS-010-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-MLOPS-010-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-MLOPS-010-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Controlled retraining triggers and approvals | Defined in canonical portfolio |
