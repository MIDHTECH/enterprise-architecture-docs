...
**Model Rollback** addresses a specific operating need inside the
Enterprise MLOps Model Platform: **Safe recovery to a prior approved model**. Without a shared design, teams can perform
...
For an approved scope, the future workflow evaluates **Model Rollback** through
...
| --- | --- |
| Owning platform | Model Rollback turns a versioned platform intent into a repeatable decision rather than an isolated operator action. |
...
1. The request identifies **Model Rollback**, the enterprise outcome, owner, immutable
...
```mermaid
flowchart LR
    Need["Enterprise need and owner"] --> Contract["Versioned Model Rollback contract"]
...
These considerations make the page specific to **Model Rollback** while preserving the
same enterprise control language used across the owning platform. In particular,
the later implementation must demonstrate recovery integrity, failure transition and return, model and data lineage, lifecycle governance.
...
| Planned `UC-MLOPS-011/contract` | Define inputs, owner, target allowlist, mode, coverage, policy, outputs, and safe stop |
| Planned `UC-MLOPS-011/result-schema` | Normalize provenance, observed values, decision, safety, recovery, and review fields |
| Planned `UC-MLOPS-011/fixtures` | Exercise passing, blocking, malformed, unauthorized, unavailable-dependency, and recovery cases |
...
**Description:** The platform owner and enterprise consumer need Model Rollback defined
