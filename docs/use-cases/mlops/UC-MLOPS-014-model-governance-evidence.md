...
ML engineer, MLOps engineer, data owner, governance reviewer, workflow owner, and SRE need a repeatable, auditable implementation of **Model Governance Evidence**.
The canonical coverage target is: Audit records for regulated model operation. The work must strengthen the
...
evidence, and safe stop for Model Governance Evidence. CI proves source behavior with
...
| --- | --- |
| `use-cases/UC-MLOPS-014/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-MLOPS-014/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-MLOPS-014/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-MLOPS-014/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-MLOPS-014.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-MLOPS-014-001: Define and validate the platform contract

**Description:** The platform owner needs Model Governance Evidence expressed as versioned
...
sensitive inputs are rejected, and the expected coverage is: Audit records for regulated model operation.
...
**Required attachments:** `ART-MLOPS-014-001A` contract validation
and fixture report.

### STORY-MLOPS-014-002: Execute safely and prove the outcome

**Description:** Platform and enterprise owners need a reproducible result for
Model Governance Evidence that distinguishes source completion from runtime acceptance and
...
**Required attachments:** `ART-MLOPS-014-002A` execution result,
`ART-MLOPS-014-002B` rollback or non-mutation proof, and
`ATT-MLOPS-014-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-MLOPS-014-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-MLOPS-014-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-MLOPS-014-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-MLOPS-014-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Audit records for regulated model operation | Defined in canonical portfolio |
