# UC-MLOPS-012: Experiment Tracking
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | Experiment Tracking |
| Primary platform | Enterprise MLOps Model Platform |
| Supporting use cases | [UC-MLOPS-002](UC-MLOPS-002-ml-training-pipeline-standardization.md), [UC-MLOPS-001](UC-MLOPS-001-model-registry-versioning.md), [UC-DATA-014](../data/UC-DATA-014-data-lineage.md), [UC-GOV-001](../governance/UC-GOV-001-compliance-evidence-collection.md) |
...
| Jira epic | `EPIC-MLOPS-012` — Implement Experiment Tracking |
...
Experiment Tracking governs **Metrics, artifacts, parameters and reproducibility** through reproducible model, data, code, and decision lineage.

For Experiment Tracking, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Metrics, artifacts, parameters and reproducibility** on the documented
...
Acceptance for Experiment Tracking requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise MLOps Model Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Experiment Tracking. |
...
- The implementation contract defines the scope required to demonstrate:
  **Metrics, artifacts, parameters and reproducibility**
...
- the versioned contract for Experiment Tracking;
...
For design review, walk through Experiment Tracking by trying to keep dataset, code, parameters,
...
including on rejected and replayed paths. In this page, **UC-MLOPS-002: ML Training Pipeline
Standardization** contributes reproducible lifecycle decision with promotion, rollback, or
retirement eligibility; **UC-MLOPS-001: Model Registry and Versioning** contributes model
artifact identity, lineage, and lifecycle state. The first buildable boundary is existing GitLab
...
Experiment Tracking is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Metrics, artifacts, parameters and reproducibility**—rather than a new product or
environment.
...
Experiment Tracking. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-MLOPS-012 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-MLOPS-012/UC-MLOPS-012-architecture.svg)
...
Experiment Tracking remains accountable to its primary platform. The dependencies below
...
Before Experiment Tracking is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For Experiment Tracking, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Metrics, artifacts, parameters and reproducibility** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Experiment Tracking become
...
The Experiment Tracking design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Experiment Tracking | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Experiment Tracking, the primary threat is **untrusted code, model, data, or dependency content entering a promoted artifact**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-MLOPS-012 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Experiment Tracking implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/ai-and-ml-platform/mlops-model-platform/contracts/uc-mlops-012.yaml` |
| Primary implementation | `midhhealth/ai-and-ml-platform/mlops-model-platform/src/lifecycle/experiment_tracking.py`; entry point: the `run_experiment_tracking` lifecycle evaluator |
| Machine-readable result schema | `midhhealth/ai-and-ml-platform/mlops-model-platform/schemas/uc-mlops-012-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/ai-and-ml-platform/mlops-model-platform/tests/fixtures/uc-mlops-012/` |
| GitLab source gate | `midhhealth/ai-and-ml-platform/mlops-model-platform/.gitlab/ci/uc-mlops-012.yml` |
| Operator diagnosis and recovery | `midhhealth/ai-and-ml-platform/mlops-model-platform/docs/runbooks/uc-mlops-012.md` |
...
3. **Read-only proof:** execute the `run_experiment_tracking` lifecycle evaluator, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/ai-and-ml-platform/mlops-model-platform/contracts/uc-mlops-012.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/ai-and-ml-platform/mlops-model-platform/src/lifecycle/experiment-tracking.py` | Primary implementation through the `run_experiment_tracking` lifecycle evaluator | Planned |
| `midhhealth/ai-and-ml-platform/mlops-model-platform/schemas/uc-mlops-012-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/ai-and-ml-platform/mlops-model-platform/tests/fixtures/uc-mlops-012/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/ai-and-ml-platform/mlops-model-platform/.gitlab/ci/uc-mlops-012.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/ai-and-ml-platform/mlops-model-platform/docs/runbooks/uc-mlops-012.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-MLOPS-012-001: Define the Experiment Tracking contract
...
**Implementation steps:** Write `midhhealth/ai-and-ml-platform/mlops-model-platform/docs/runbooks/uc-mlops-012.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-MLOPS-012-001A` contract and fixture review.

### STORY-MLOPS-012-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Experiment Tracking, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-MLOPS-012-002A` source validation and
`ART-MLOPS-012-002B` downstream-block proof.

### STORY-MLOPS-012-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Metrics, artifacts, parameters and reproducibility** on its named scope without hidden
...
**Required attachments:** Future `ART-MLOPS-012-003A` bounded result,
`ART-MLOPS-012-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-MLOPS-012-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-MLOPS-012-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-MLOPS-012-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-MLOPS-012-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-MLOPS-012-003A` | Bounded observed result compared with **Metrics, artifacts, parameters and reproducibility** | Approved existing execution path | Pending future execution |
| `ART-MLOPS-012-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Metrics, artifacts, parameters and reproducibility | Detailed behavior and decision semantics documented |
