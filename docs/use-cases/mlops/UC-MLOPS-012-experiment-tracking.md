...
ML engineer, MLOps engineer, data owner, governance reviewer, workflow owner, and SRE need a repeatable, auditable implementation of **Experiment Tracking**.
The canonical coverage target is: Metrics, artifacts, parameters and reproducibility. The work must strengthen the
...
evidence, and safe stop for Experiment Tracking. CI proves source behavior with
...
| --- | --- |
| `use-cases/UC-MLOPS-012/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-MLOPS-012/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-MLOPS-012/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-MLOPS-012/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-MLOPS-012.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-MLOPS-012-001: Define and validate the platform contract

**Description:** The platform owner needs Experiment Tracking expressed as versioned
...
sensitive inputs are rejected, and the expected coverage is: Metrics, artifacts, parameters and reproducibility.
...
**Required attachments:** `ART-MLOPS-012-001A` contract validation
and fixture report.

### STORY-MLOPS-012-002: Execute safely and prove the outcome

**Description:** Platform and enterprise owners need a reproducible result for
Experiment Tracking that distinguishes source completion from runtime acceptance and
...
**Required attachments:** `ART-MLOPS-012-002A` execution result,
`ART-MLOPS-012-002B` rollback or non-mutation proof, and
`ATT-MLOPS-012-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-MLOPS-012-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-MLOPS-012-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-MLOPS-012-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-MLOPS-012-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Metrics, artifacts, parameters and reproducibility | Defined in canonical portfolio |
