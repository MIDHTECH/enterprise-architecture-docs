...
Delivery engineer, application or platform owner, security reviewer, and SRE need a repeatable, auditable implementation of **Deployment Health Scoring**.
The canonical coverage target is: Release gates score health, SLO burn, alerts and rollback readiness. The work must strengthen the
...
| --- | --- |
| `use-cases/UC-CICD-015/contract.yml` | Trigger, enterprise outcome, owner, target allowlist, safety, and evidence contract |
| `use-cases/UC-CICD-015/schemas/report.schema.json` | Machine-readable result and provenance requirements |
| `use-cases/UC-CICD-015/scripts/execute.sh` | Read-only plan or approved bounded action with explicit exit codes |
| `use-cases/UC-CICD-015/tests/` | Passing, failing, denied-target, redaction, and rollback fixtures |
| `.gitlab-ci.yml` | Pinned validation job on an existing accepted runner |
| `docs/use-cases/UC-CICD-015.md` | Implementation, operations, troubleshooting, and evidence notes |

## Jira breakdown

### STORY-CICD-015-001: Define and validate the platform contract
...
sensitive inputs are rejected, and the expected coverage is: Release gates score health, SLO burn, alerts and rollback readiness.
...
**Required attachments:** `ART-CICD-015-001A` contract validation
and fixture report.

### STORY-CICD-015-002: Execute safely and prove the outcome
...
**Required attachments:** `ART-CICD-015-002A` execution result,
`ART-CICD-015-002B` rollback or non-mutation proof, and
`ATT-CICD-015-002A` only when a real UI capture adds review value.

## Evidence and screenshot register

| ID | Evidence | Source | Status |
| --- | --- | --- | --- |
| `ART-CICD-015-001A` | Contract, reference, and fixture validation | GitLab CI | Pending |
| `ART-CICD-015-002A` | Bounded execution or read-only result | Jenkins, AWX, GitLab CI, or approved API | Pending |
| `ART-CICD-015-002B` | Rollback, recovery, idempotence, or non-mutation proof | Approved execution path | Pending |
| `ATT-CICD-015-002A` | Optional sanitized supporting capture | Named source system | Pending if required |

## Expected versus current result

| Measure | Expected | Current |
| --- | --- | --- |
| Platform fit | Release gates score health, SLO burn, alerts and rollback readiness | Defined in canonical portfolio |
