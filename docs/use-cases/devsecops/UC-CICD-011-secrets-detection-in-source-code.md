# UC-CICD-011: Secrets Detection in Source Code
...
| Field | Value |
| --- | --- |
| Canonical portfolio use case | Secrets Detection in Source Code |
...
Secrets Detection in Source Code defines how reviewed source moves through **Pipeline can run Gitleaks/TruffleHog-style checks** without bypassing security, evidence, or recovery gates.

For Secrets Detection in Source Code, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Pipeline can run Gitleaks/TruffleHog-style checks** on the documented
...
Acceptance for Secrets Detection in Source Code requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise DevSecOps Delivery Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Secrets Detection in Source Code. |
...
- The implementation contract defines the scope required to demonstrate:
  **Pipeline can run Gitleaks/TruffleHog-style checks**
...
- the versioned contract for Secrets Detection in Source Code;
...
For design review, walk through Secrets Detection in Source Code by trying to follow one
reviewed change from commit to an identifiable release decision. The result MidhHealth needs is
to deliver reviewed changes safely to provider, payer, and shared platform services. Enterprise
...
first buildable boundary is existing GitLab, accepted runners, Jenkins, AWX, and Kubernetes
...
Secrets Detection in Source Code is evaluated inside the existing enterprise lab and the owning
platform's current source-control and execution boundaries. The architectural
unit is the governed outcome—**Pipeline can run Gitleaks/TruffleHog-style checks**—rather than a new product or
environment.
...
Secrets Detection in Source Code. It does not absorb the responsibilities of the dependency use cases
listed below.

## Architecture diagram

![UC-CICD-011 architecture showing demand, source contracts, planned control, existing target, evidence, and recovery](../../assets/use-cases/UC-CICD-011/UC-CICD-011-architecture.svg)
...
Secrets Detection in Source Code remains accountable to its primary platform. The dependencies below
...
Before Secrets Detection in Source Code is implemented, every handoff must resolve to an immutable
revision and machine-readable artifact. A URL, screenshot, or verbal approval
alone is not sufficient dependency evidence.

## Quality attributes

For Secrets Detection in Source Code, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Pipeline can run Gitleaks/TruffleHog-style checks** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Secrets Detection in Source Code become
...
The Secrets Detection in Source Code design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Secrets Detection in Source Code | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Secrets Detection in Source Code, the primary threat is **untrusted source or dependency content reaching a privileged runner**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-CICD-011 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Secrets Detection in Source Code implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/contracts/uc-cicd-011.yaml` |
| Primary implementation | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/jobs/secrets-detection-in-source-code.groovy`; entry point: the `secrets-detection-in-source-code` Jenkins job and its shared-library step |
| Machine-readable result schema | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/schemas/uc-cicd-011-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/tests/fixtures/uc-cicd-011/` |
| GitLab source gate | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/.gitlab/ci/uc-cicd-011.yml` |
| Operator diagnosis and recovery | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/docs/runbooks/uc-cicd-011.md` |
...
3. **Read-only proof:** execute the `secrets-detection-in-source-code` Jenkins job and its shared-library step, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/contracts/uc-cicd-011.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/jobs/secrets-detection-in-source-code.groovy` | Primary implementation through the `secrets-detection-in-source-code` Jenkins job and its shared-library step | Planned |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/schemas/uc-cicd-011-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/tests/fixtures/uc-cicd-011/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/.gitlab/ci/uc-cicd-011.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/docs/runbooks/uc-cicd-011.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-CICD-011-001: Define the Secrets Detection in Source Code contract
...
**Implementation steps:** Write `midhhealth/platform-delivery/devsecops-cicd-orchestrator/docs/runbooks/uc-cicd-011.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-CICD-011-001A` contract and fixture review.

### STORY-CICD-011-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Secrets Detection in Source Code, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-CICD-011-002A` source validation and
`ART-CICD-011-002B` downstream-block proof.

### STORY-CICD-011-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Pipeline can run Gitleaks/TruffleHog-style checks** on its named scope without hidden
...
**Required attachments:** Future `ART-CICD-011-003A` bounded result,
`ART-CICD-011-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-CICD-011-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-CICD-011-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-CICD-011-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-CICD-011-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-CICD-011-003A` | Bounded observed result compared with **Pipeline can run Gitleaks/TruffleHog-style checks** | Approved existing execution path | Pending future execution |
| `ART-CICD-011-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Pipeline can run Gitleaks/TruffleHog-style checks | Detailed behavior and decision semantics documented |
