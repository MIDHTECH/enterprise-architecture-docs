...
**Secrets Detection in Source Code** addresses a specific operating need inside the
Enterprise DevSecOps Delivery Platform: **Pipeline can run Gitleaks/TruffleHog-style checks**. Without a shared design, teams can perform
...
For an approved scope, the future workflow evaluates **Secrets Detection in Source Code** through
...
| --- | --- |
| Owning platform | Secrets Detection in Source Code turns a versioned platform intent into a repeatable decision rather than an isolated operator action. |
...
1. The request identifies **Secrets Detection in Source Code**, the enterprise outcome, owner, immutable
...
```mermaid
flowchart LR
    Need["Enterprise need and owner"] --> Contract["Versioned Secrets Detection in Source Code contract"]
...
These considerations make the page specific to **Secrets Detection in Source Code** while preserving the
...
| Planned `UC-CICD-011/contract` | Define inputs, owner, target allowlist, mode, coverage, policy, outputs, and safe stop |
| Planned `UC-CICD-011/result-schema` | Normalize provenance, observed values, decision, safety, recovery, and review fields |
| Planned `UC-CICD-011/fixtures` | Exercise passing, blocking, malformed, unauthorized, unavailable-dependency, and recovery cases |
...
**Description:** The platform owner and enterprise consumer need Secrets Detection in Source Code defined
