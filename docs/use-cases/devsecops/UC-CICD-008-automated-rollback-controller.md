# UC-CICD-008: Automated Rollback Controller
...
| Jira epic | `EPIC-CICD-008` — Implement Automated Rollback Controller |
...
Automated Rollback Controller defines how reviewed source moves through **Pipeline reverses deployments when health or SLO checks fail** without bypassing security, evidence, or recovery gates.

For Automated Rollback Controller, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Pipeline reverses deployments when health or SLO checks fail** on the documented
...
Acceptance for Automated Rollback Controller requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise DevSecOps Delivery Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Automated Rollback Controller. |
...
- The implementation contract defines the scope required to demonstrate:
  **Pipeline reverses deployments when health or SLO checks fail**
...
- the versioned contract for Automated Rollback Controller;
...
unit is the governed outcome—**Pipeline reverses deployments when health or SLO checks fail**—rather than a new product or
environment.
...
Automated Rollback Controller. It does not absorb the responsibilities of the dependency use cases
listed below.
...
flowchart LR
    ...
    subgraph Planned["Planned Automated Rollback Controller control"]
...
1. The request binds Automated Rollback Controller to immutable source, an inventory-resolved target,
...
- The canonical coverage test is: **Pipeline reverses deployments when health or SLO checks fail**
...
Automated Rollback Controller remains accountable to its primary platform. The dependencies below
...
For Automated Rollback Controller, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Pipeline reverses deployments when health or SLO checks fail** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Automated Rollback Controller become
...
The Automated Rollback Controller design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Automated Rollback Controller | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Automated Rollback Controller, the primary threat is **untrusted source or dependency content reaching a privileged runner**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-CICD-008 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Automated Rollback Controller implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/contracts/uc-cicd-008.yaml` |
| Primary implementation | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/jobs/automated-rollback-controller.groovy`; entry point: the `automated-rollback-controller` Jenkins job and its shared-library step |
| Machine-readable result schema | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/schemas/uc-cicd-008-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/tests/fixtures/uc-cicd-008/` |
| GitLab source gate | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/.gitlab/ci/uc-cicd-008.yml` |
| Operator diagnosis and recovery | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/docs/runbooks/uc-cicd-008.md` |
...
3. **Read-only proof:** execute the `automated-rollback-controller` Jenkins job and its shared-library step, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/contracts/uc-cicd-008.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/jobs/automated-rollback-controller.groovy` | Primary implementation through the `automated-rollback-controller` Jenkins job and its shared-library step | Planned |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/schemas/uc-cicd-008-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/tests/fixtures/uc-cicd-008/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/.gitlab/ci/uc-cicd-008.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/docs/runbooks/uc-cicd-008.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-CICD-008-001: Define the Automated Rollback Controller contract
...
**Implementation steps:** Write `midhhealth/platform-delivery/devsecops-cicd-orchestrator/docs/runbooks/uc-cicd-008.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-CICD-008-001A` contract and fixture review.

### STORY-CICD-008-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Automated Rollback Controller, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-CICD-008-002A` source validation and
`ART-CICD-008-002B` downstream-block proof.

### STORY-CICD-008-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Pipeline reverses deployments when health or SLO checks fail** on its named scope without hidden
...
**Required attachments:** Future `ART-CICD-008-003A` bounded result,
`ART-CICD-008-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-CICD-008-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-CICD-008-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-CICD-008-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-CICD-008-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-CICD-008-003A` | Bounded observed result compared with **Pipeline reverses deployments when health or SLO checks fail** | Approved existing execution path | Pending future execution |
| `ART-CICD-008-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Pipeline reverses deployments when health or SLO checks fail | Detailed behavior and decision semantics documented |
