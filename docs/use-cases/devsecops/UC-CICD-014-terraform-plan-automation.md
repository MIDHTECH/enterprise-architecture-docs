...
**Terraform Plan Automation** addresses a specific operating need inside the
Enterprise DevSecOps Delivery Platform: **Merge requests publish reviewed Terraform plan artifacts**. Without a shared design, teams can perform
...
For an approved scope, the future workflow evaluates **Terraform Plan Automation** through
...
| --- | --- |
| Owning platform | Terraform Plan Automation turns a versioned platform intent into a repeatable decision rather than an isolated operator action. |
...
1. The request identifies **Terraform Plan Automation**, the enterprise outcome, owner, immutable
...
```mermaid
flowchart LR
    Need["Enterprise need and owner"] --> Contract["Versioned Terraform Plan Automation contract"]
...
These considerations make the page specific to **Terraform Plan Automation** while preserving the
...
| Planned `UC-CICD-014/contract` | Define inputs, owner, target allowlist, mode, coverage, policy, outputs, and safe stop |
| Planned `UC-CICD-014/result-schema` | Normalize provenance, observed values, decision, safety, recovery, and review fields |
| Planned `UC-CICD-014/fixtures` | Exercise passing, blocking, malformed, unauthorized, unavailable-dependency, and recovery cases |
...
**Description:** The platform owner and enterprise consumer need Terraform Plan Automation defined
