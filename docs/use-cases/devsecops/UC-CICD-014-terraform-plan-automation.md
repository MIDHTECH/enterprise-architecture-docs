# UC-CICD-014: Terraform Plan Automation
...
| Jira epic | `EPIC-CICD-014` — Implement Terraform Plan Automation |
...
Terraform Plan Automation defines how reviewed source moves through **Merge requests publish reviewed Terraform plan artifacts** without bypassing security, evidence, or recovery gates.

For Terraform Plan Automation, the design fixes the contract, dependency handoffs, target boundary, evidence, decision owners, and recovery path before implementation. Those choices keep the eventual build grounded in the lab that actually exists.

## Expected outcome

The first delivery slice proves **Merge requests publish reviewed Terraform plan artifacts** on the documented
...
Acceptance for Terraform Plan Automation requires rejected cases to stop safely and unrelated
...
| --- | --- |
| Owning responsibility | **Enterprise DevSecOps Delivery Platform** owns the contract, control behavior, evidence schema, and recovery boundary for Terraform Plan Automation. |
...
- The implementation contract defines the scope required to demonstrate:
  **Merge requests publish reviewed Terraform plan artifacts**
...
- the versioned contract for Terraform Plan Automation;
...
unit is the governed outcome—**Merge requests publish reviewed Terraform plan artifacts**—rather than a new product or
environment.
...
Terraform Plan Automation. It does not absorb the responsibilities of the dependency use cases
listed below.
...
flowchart LR
    ...
    subgraph Planned["Planned Terraform Plan Automation control"]
...
1. The request binds Terraform Plan Automation to immutable source, an inventory-resolved target,
...
- The canonical coverage test is: **Merge requests publish reviewed Terraform plan artifacts**
...
Terraform Plan Automation remains accountable to its primary platform. The dependencies below
...
For Terraform Plan Automation, quality is measured against the bounded enterprise outcome—not
document length or a green job. Unapproved business thresholds remain explicit
decisions and must not be invented.

| Attribute | Required measure or invariant | Decision state |
| --- | --- | --- |
| Functional correctness | Every required input is validated; **Merge requests publish reviewed Terraform plan artifacts** is evaluated against positive, negative, missing-input, and unauthorized-scope cases. | Fixed design requirement |
...
Load, latency, availability, retention, RTO, and RPO values for Terraform Plan Automation become
...
The Terraform Plan Automation design separates source validation, privileged execution, target
...
| Executor → existing target | Minimum commands or API operations required for Terraform Plan Automation | Least-privilege identity, explicit target limit, timeout, and stop condition |
| Target → evidence store | Sanitized metadata, measurements, decision, and recovery result | Exclude credentials, tokens, private keys, kubeconfigs, packet payloads, PHI, PII, and unrelated records |

For Terraform Plan Automation, the primary threat is **untrusted source or dependency content reaching a privileged runner**. The mandatory response is
...
If any selected approach changes, record the rationale beside UC-CICD-014 in
the implementation repository before code review. A documentation edit alone
does not approve the new architecture.

## Implementation design

The first Terraform Plan Automation implementation is deliberately source-only. Its planned files live in the existing repository; none provisions infrastructure.

| Planned source responsibility | Exact planned location |
| --- | --- |
| Use-case contract and target allowlist | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/contracts/uc-cicd-014.yaml` |
| Primary implementation | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/jobs/terraform-plan-automation.groovy`; entry point: the `terraform-plan-automation` Jenkins job and its shared-library step |
| Machine-readable result schema | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/schemas/uc-cicd-014-result.schema.json` |
| Positive, negative, malformed, and recovery fixtures | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/tests/fixtures/uc-cicd-014/` |
| GitLab source gate | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/.gitlab/ci/uc-cicd-014.yml` |
| Operator diagnosis and recovery | `midhhealth/platform-delivery/devsecops-cicd-orchestrator/docs/runbooks/uc-cicd-014.md` |
...
3. **Read-only proof:** execute the `terraform-plan-automation` Jenkins job and its shared-library step, publish a checksummed result, and
...
| --- | --- | --- |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/contracts/uc-cicd-014.yaml` | Inputs, owner, dependency revisions, target allowlist, modes, thresholds, and stop conditions | Planned |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/jobs/terraform-plan-automation.groovy` | Primary implementation through the `terraform-plan-automation` Jenkins job and its shared-library step | Planned |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/schemas/uc-cicd-014-result.schema.json` | Provenance, observations, decision, reason codes, safety, and recovery result | Planned |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/tests/fixtures/uc-cicd-014/` | Passing, blocking, malformed, unauthorized, stale-dependency, and recovery cases | Planned |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/.gitlab/ci/uc-cicd-014.yml` | Source validation on an accepted existing runner | Planned |
| `midhhealth/platform-delivery/devsecops-cicd-orchestrator/docs/runbooks/uc-cicd-014.md` | Preconditions, execution, diagnosis, evidence review, safe stop, and recovery | Planned |
...
### STORY-CICD-014-001: Define the Terraform Plan Automation contract
...
**Implementation steps:** Write `midhhealth/platform-delivery/devsecops-cicd-orchestrator/docs/runbooks/uc-cicd-014.md`; reconfirm inventory and dependency evidence; run source and read-only modes; obtain separate approval for one canary if mutation is required; collect the schema-valid result, independent post-check, recovery proof, and owner decision.
...
**Required attachments:** Future `ART-CICD-014-001A` contract and fixture review.

### STORY-CICD-014-002: Build the source and evidence gate

**Description:** The implementation owner needs a fail-closed source workflow
that evaluates Terraform Plan Automation, produces normalized evidence, and controls only its
...
**Required attachments:** Future `ART-CICD-014-002A` source validation and
`ART-CICD-014-002B` downstream-block proof.

### STORY-CICD-014-003: Verify the bounded outcome and recovery

**Description:** Platform and enterprise reviewers need evidence that the
future workflow satisfies **Merge requests publish reviewed Terraform plan artifacts** on its named scope without hidden
...
**Required attachments:** Future `ART-CICD-014-003A` bounded result,
`ART-CICD-014-003B` recovery/non-mutation proof, and an optional sanitized
`ATT-CICD-014-003A` only when a real capture adds review value.

## Evidence and screenshot register

| ID | Planned evidence | Source | Current status |
| --- | --- | --- | --- |
| `ART-CICD-014-001A` | Reviewed contract, inventory, dependency, and fixture design | Architecture and implementation repository review | Pending future implementation |
| `ART-CICD-014-002A` | Positive and negative source-gate results | Existing GitLab and accepted runner | Pending future execution |
| `ART-CICD-014-002B` | Failed-decision proof showing the declared downstream path blocked | Existing delivery pipeline | Pending future execution |
| `ART-CICD-014-003A` | Bounded observed result compared with **Merge requests publish reviewed Terraform plan artifacts** | Approved existing execution path | Pending future execution |
| `ART-CICD-014-003B` | Recovery, restore, idempotence, reconciliation, or zero-change proof | Approved existing execution path | Pending future execution |

## Expected versus current result

| Measure | Expected future result | Current result |
| --- | --- | --- |
| Canonical coverage | Merge requests publish reviewed Terraform plan artifacts | Detailed behavior and decision semantics documented |
