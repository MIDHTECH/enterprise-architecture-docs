...
| 2026-07-28 | `infra03.example.com` | Rebuilt additional disk for VM migration | Explicitly erased legacy `/dev/sdb` data; created GPT/ext4 `/dev/sdb1` labeled `infra03-libvirt`, mounted it at `/var/lib/libvirt/infra03-images`, and validated persistent/autostart pool `infra03-images` with 3.58 TiB available |
...
| 2026-07-25 | `gitlab.example.com` | Created local source-control namespace | Private GitLab group and eight private projects created; all repository origins restricted to local GitLab |
...
| 2026-07-28 | Live platform audit | Reconciled hypervisors, 31 VMs, product services, Kubernetes, repositories, and use-case evidence | Verified 22 active VM roles, 9 provisioned-only product VMs, four Ready Kubernetes nodes, 20 synchronized repositories, and nine explicitly implemented first slices |
| 2026-07-28 | NGINX and DNS | Reconciled proxy catalog with live services | Corrected AWX to `32000`, added Headlamp, corrected MinIO API routing, restricted backend firewall access to `.114`, introduced intentional 503 responses, and proved zero-change convergence; see INC-2026-023 |
| 2026-07-28 | Elastic ingestion | Audited indexed source coverage | Found only three synthetic AWX verification events and no Rocky Linux fleet logs; opened INC-2026-024 |
| 2026-07-28 | Rocky Linux fleet logging | Deployed Filebeat 9.4.2 with TLS and disk queues | Thirty reachable VMs passed service and encrypted-output checks; Elasticsearch proved the same 30 hostnames; AWX remains pending under INC-2026-024 |
| 2026-07-28 | Elastic ingestion | Processed first-enrollment history import | Logstash handled 518,971 events and its persistent queue drained to zero; documented active-file backfill behavior under INC-2026-025 |
| 2026-07-28 | Fleet verification | Recovered from transient Elasticsearch01 SSH disconnect | Direct retry confirmed SSH, green three-node Elasticsearch health, and 30-host coverage; see INC-2026-026 |
| 2026-07-28 | AWX access recovery | Attempted non-disruptive QEMU Guest Agent key repair | Guest agent was healthy, but existing `.ssh` ownership/modes blocked read and additive repair; documented one-console-session fix under INC-2026-027 |
| 2026-07-28 | AWX and fleet-log acceptance | Restored canonical SSH, enrolled the final sender, and ran the full verifier | All 31 Filebeat services and encrypted outputs passed; Elasticsearch returned 31/31 hostnames, no missing hosts, and at least 10,000 recent events; closed INC-2026-024 and INC-2026-027 |
| 2026-07-28 | Application and telemetry readiness | Audited hypervisors, Kubernetes deployment prerequisites, management URLs, and logs-metrics-traces pipelines | 31/31 VMs and 4/4 Kubernetes nodes are running; Elastic host logging is accepted at 31/31; Prometheus covers 27/31 hosts; Kubernetes storage and ingress, Grafana provisioning, Loki streams, and Tempo traces remain incomplete; opened INC-2026-030 for missing Headlamp DNS |
| 2026-07-28 | Prometheus and Grafana reconciliation | Refreshed the stale AWX production inventory and ran `deploy-prometheus-stack` job 321 | Job completed with zero failures or unreachable hosts; Prometheus reports 32/32 targets Up—31 Node Exporters plus self—and Grafana has its Prometheus data source and fleet dashboard; resolved INC-2026-031 |
| 2026-07-29 | Grafana reconciliation | Removed the redundant Node Exporter download dependency and ran AWX job 356 | Job 356 succeeded; Grafana provisioned Prometheus, Loki, and Tempo data sources and verified healthy; resolved INC-2026-035 |
| 2026-07-29 | AWX execution policy | Changed all execution environments to pull cached images unless missing | Inventory update 353 and subsequent deployment jobs launched without a Quay pull; resolved INC-2026-034 |
| 2026-07-29 | Kubernetes and observability data plane | Added managed OTLP/Loki/Tempo firewalls, BIND pod-CIDR ACL, and deterministic CoreDNS private-zone forwarding | Repeated private-zone lookups succeeded on all three workers; resolved INC-2026-036 through INC-2026-038 |
| 2026-07-29 | Correlated telemetry acceptance | Ran bounded AWX workflow job 381 | Run `20260729T164944Z` produced Tempo trace `f819ea257b72ff3a7fd5998e807b3430`; trace retrieval passed and Loki returned one stream with the same trace ID |
| 2026-07-29 | GitLab CI scheduler | Investigated pending jobs with an active, valid, idle runner | Restart and bounded single-runner recovery did not claim jobs; pipeline 307 remains pending under INC-2026-039 |
| 2026-07-29 | GitLab Runner scope | Converted the project-scoped runner to an instance runner and tuned two concurrent workers | Runner ID 2 concurrently claimed projects 2 and 15; pipeline 307 left pending, project 2 job 498 succeeded, and INC-2026-039 was resolved |
| 2026-07-29 | GitLab Runner closure | Published the incident recovery documentation and observed its fresh CI run | Enterprise-documentation pipeline 310 for commit `aac120f` was assigned to runner ID 2 and passed |
| 2026-07-29 | GitLab CI runtime repair | Added explicit Terraform and Ansible images, one cloud stage graph, repository role paths, and collection dependencies | Observability pipeline 315 passed; cloud pipeline 317 passed format, both Terraform validations, layout, and Ansible lint; resolved INC-2026-040 |
| 2026-07-29 | Terraform policy gate | Ran the first complete Checkov scan against the legacy cloud examples | 77 controls passed and 14 remained blocking; recorded remediation under INC-2026-041 and kept automatic plan/smoke execution disabled |
| 2026-07-29 | Terraform policy remediation | Implemented image, IAM, network, flow-log, KMS, S3 logging, lifecycle, notification, and replication controls without skips | Pipeline 327 job 672 passed 153 controls with two KMS-policy findings; pipeline 329 job 696 passed 171 with six KMS policy-classification findings; final pipeline 332 job 730 passed 155 controls with zero failures and zero skips; INC-2026-041 resolved |
| 2026-07-29 | infra01/GitLab publication path | Detected infra01 and GitLab absent from the LAN during final Checkov publication | Mac reached router and infra02 but had rejected/incomplete neighbor state for GitLab; infra02 independently reported `FAILED` neighbors for infra01 `.38` and GitLab `.101`; after recovery infra01 showed almost four days of uninterrupted uptime, proving a network-path interruption rather than host power loss; tracked as an INC-2026-033 recurrence |
| 2026-07-29 | GitLab Runner image resilience | Aligned pinned project images with a cached-first runner policy | Pipeline 322 jobs 614 and 615 passed from the cached Terraform image; runner permits explicit `always` and retains a timestamped pre-change backup; resolved INC-2026-042 |
| 2026-07-29 | AWX cloud/DNS onboarding | Created AWX project 23, inventory 4, source 24, and DNS-only template 25 | Initial project update 389 exposed a missing GitLab deploy-key assignment; enabled the existing key read-only and project update 390 plus inventory update 391 succeeded; resolved INC-2026-043 |
| 2026-07-29 | Headlamp DNS deployment | Added `headlamp.apps.example.com` at `.114`, advanced zone serial, and added a managed lookup assertion | Initial AWX job 394 exposed missing root role discovery; commit `8ff344a` and pipeline 337 corrected it; job 398 deployed successfully and job 402 converged with zero changes; BIND, Mac, CoreDNS, and HTTP 200 acceptance passed; resolved INC-2026-030 and INC-2026-044 |
| 2026-07-29 | AWX inventory normalization | Split physical KVM hosts from product VMs, moved DNS/NGINX templates to canonical `production`, and added the missing product groups | Cloud pipeline 346, inventory pipeline 347, and source sync 425 passed; AWX now has 38 records/34 distinct names; DNS jobs 433/438 and NGINX jobs 443/448 processed one host with zero changes and failures; resolved INC-2026-049 and INC-2026-051 |
| 2026-07-31 | AWX direct canonical URL | Replaced AWX's shared-proxy route with NGINX on `awx.example.com`, removed only `awx.apps.example.com`, and tightened the DNS template to `dns.example.com` | Cloud pipelines 378/380 passed commit `e8873211`; AWX sync 484 selected canonical main; proxy jobs 485/492 and DNS jobs 502/514 succeeded with clean second convergence; authenticated `http://awx.example.com` dashboard accepted; shared proxy retained for all other routes |
