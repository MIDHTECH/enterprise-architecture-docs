...
| Permitted work | Read-only diagnosis, idle-interval reproduction, and publication of the PLAN 9/pre-DEPLOY recurrence evidence; review and publish the bounded [infra03 physical-path canary](change-records/CHG-2026-011-infra03-physical-path-canary.md). Preserve all VMs and the healthy cluster. Physical execution requires local-console presence and the exact labeled target; do not run another Argo CD PLAN or DEPLOY until the path is corrected and accepted across separated observation windows. |
